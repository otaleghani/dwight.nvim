...
local function fetch_url(url, callback)
	...
	handle = uv.spawn("curl", {
		...
		vim.schedule(function()
			...
			local text = raw:gsub("<script.-</script>", "")
				:gsub("<style.-</style>", "")
				:gsub("<[^>]+>", " ")
				:gsub("&nbsp;", " ")
				:gsub("&amp;", "&")
	...
	stderr:read_start(function(e, d)
		if not e and d then
			stderr_chunks[#stderr_chunks + 1] = d
		end
	end)
end
...
local function find_package_types(pkg_name)
	...
	local py_paths =
		vim.fn.systemlist('python3 -c "import ' .. pkg_name .. "; print(" .. pkg_name .. '.__file__)" 2>/dev/null')
	if #py_paths > 0 and py_paths[1] ~= "" then
		local pkg_dir = vim.fn.fnamemodify(py_paths[1], ":h")
		local pyi = pkg_dir .. "/__init__.pyi"
		if vim.fn.filereadable(pyi) == 1 then
			return pyi, "python"
		end
...
local function extract_signatures_from_file(path, lang)
	...
	if lang == "python" then
		...
		for line in content:gmatch("[^\n]+") do
			if
				line:match("^%s*def ")
				or line:match("^%s*class ")
				or line:match("^%s*async def ")
				or line:match("^%s*@")
				or line:match('^%s*"""')
				or line:match("^%s*'''")
			then
...
function M._from_source(name)
	...
	if not sigs then
		vim.notify("[dwight] Failed to read " .. path, vim.log.levels.ERROR)
		return
	end
...
function M._from_urls(name, urls)
	...
	fetch_multiple_urls(urls, function(results)
		...
		if #all_text == 0 then
			vim.notify("[dwight] All URL fetches failed.", vim.log.levels.ERROR)
			return
		end

		local combined = table.concat(all_text, "\n\n")
		if #combined > 25000 then
			combined = combined:sub(1, 25000)
		end
...
function M._save_lib(name, raw, exit_code, job_id, source)
	...
	if xml_start then
		...
		if xml_end then
			content = content:sub(1, xml_end + 9)
		end
	end
...
function M.pick()
	local names = M.list()
	if #names == 0 then
		vim.notify("[dwight] No libs. Run :DwightAddLib.", vim.log.levels.INFO)
		return
	end

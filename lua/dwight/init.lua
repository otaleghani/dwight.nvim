...
M.defaults = {
  ...
  agentic = true,             -- DEPRECATED: agentic mode is now the only mode
...
function M.setup(opts)
  ...
  pcall(function() require_mod("languages").reset() end)

  -- Sync tracker model with backend config
  if M.config.backend == "claude_code" then
    pcall(function()
      require_mod("tracker").set_model(M.config.claude_code_model or M.config.model or "sonnet")
    end)
  end
  ...
  pcall(function() require_mod("session_log").init() end)
  ...
  if #M.config.mcp_servers > 0 then
    ...
    vim.api.nvim_create_autocmd("VimLeavePre", {
      callback = function() pcall(function() require_mod("mcp").stop_all() end) end,
    })
  end
end
...
function M._register_commands()
  ...
  cmd("DwightBootstrap", function(o)
    ...
    local opts = {}
    if args:match("%-%-quick") then opts.mode = "quick"
    elseif args:match("%-%-agentic") then opts.mode = "agentic"
    end
    if args:match("%-%-force") then opts.force = true end
    if args:match("%-%-incremental") then opts.incremental = true end
  ...
  -- Execute: DEPRECATED — legacy plan directive engine
  cmd("DwightExecute", function()
    vim.notify("[dwight] ⚠️ DwightExecute is deprecated. Use :DwightAgent instead (agentic mode).", vim.log.levels.WARN)
  end, { range = true, desc = "[DEPRECATED] Execute plan directives from buffer/selection" })
  ...
  cmd("DwightAgentResume", function()
    vim.notify("[dwight] ⚠️ DwightAgentResume is deprecated. Use :DwightAutoResume for multi-task sessions.", vim.log.levels.WARN)
  end, { desc = "[DEPRECATED] Resume agent from last failed step" })
  ...
  cmd("DwightMarketplace", function(o)
    ...
    nargs = "?",
    complete = function() return { "install", "suggest", "export", "import", "detect" } end,
    desc = "Skill marketplace: browse packs, auto-suggest, import/export",
  })
  cmd("DwightDocsFromURL", function() require_mod("skill_fetch").generate_from_url() end, { desc = "Generate skill from docs URL" })
  ...
  cmd("DwightReplay", function(o)
    ...
    complete = function()
      ...
      if not ok then return { "latest" } end
      local sessions = replay.list_sessions()
      local items = { "latest" }
      for _, s in ipairs(sessions) do items[#items + 1] = s.filename end
      return items
    end,
  ...
  cmd("DwightStats", function(o)
    ...
    nargs = "?",
    complete = function() return { "features", "export", "csv", "json" } end,
  ...
  cmd("DwightAudit", function(o)
    ...
    nargs = "?",
    complete = function() return require_mod("features").names() end,
  ...
  cmd("DwightAutoResume", function(o)
    local opts = {}
    if o.args ~= "" then opts.override_prompt = o.args end
  ...
  cmd("DwightIssue", function(o)
    ...
    if ref then
      ...
      if repo_part and num_part then
        local mode = "pick"
        if args:match("%-%-agent") then mode = "agent"
        elseif args:match("%-%-auto") then mode = "auto"
        elseif args:match("%-%-analyze") then mode = "analyze"
        end
  ...
  cmd("DwightNewIssue", function(o)
    ...
    if title and title ~= "" then opts.title = title end
  ...
  cmd("DwightCI", function(o)
    ...
    nargs = "?",
    complete = function() return { "--fix", "--rerun", "--repo" } end,
  ...
  cmd("DwightSwitch", function(o)
    ...
    nargs = 1,
    complete = function() return require_mod("providers").available_models_for_backend() end,
  ...
  cmd("DwightLessons", function(o)
      ...
      agent._consolidate_lessons(function(stats)
        ...
        elseif stats.merged_groups == 0 then
          ...
          vim.notify(string.format(
            ...
            stats.before, stats.after, stats.merged_groups, stats.removed or 0), vim.log.levels.INFO)
        end
      end)
    elseif o.args == "stats" then
      ...
      if stats.total == 0 then
        ...
        vim.notify(string.format(
          ...
          stats.total, stats.stale, stats.never_used,
          stats.avg_uses, stats.avg_reinforced), vim.log.levels.INFO)
      end
    else
      ...
      for i, l in ipairs(lessons) do
        local meta_parts = {}
        if (l.reinforced or 1) > 1 then meta_parts[#meta_parts + 1] = "×" .. l.reinforced end
        if (l.use_count or 0) > 0 then meta_parts[#meta_parts + 1] = l.use_count .. " uses" end
        if l.timestamp then
          local age = os.time() - l.timestamp
          if age > 30 * 86400 then meta_parts[#meta_parts + 1] = "⚠️ stale" end
        end
        local meta = #meta_parts > 0 and " (" .. table.concat(meta_parts, ", ") .. ")" or ""

        lines[#lines + 1] = string.format("%d. **%s** — %s%s",
          i, table.concat(l.keywords or {}, " "), l.text or "?", meta)
        if l.request then
      ...
      lines[#lines + 1] = ""
      lines[#lines + 1] = "---"
      lines[#lines + 1] = "_Commands: `:DwightLessons consolidate` | `:DwightLessons stats` | `:DwightLessons clear`_"
    ...
    nargs = "?",
    complete = function() return { "clear", "consolidate", "stats" } end,
  ...
  cmd("DwightBackend", function(o)
    ...
    if o.args == "" then
      ...
      vim.notify(string.format("[dwight] Backend: %s | Model: %s",
        cfg.backend or "claude_code", model), vim.log.levels.INFO)
    else
      ...
      if valid[o.args] then
        ...
        pcall(function() require_mod("tracker").set_model(model_map[o.args] or o.args) end)
    ...
    nargs = "?",
    complete = function() return { "claude_code", "codex", "gemini", "opencode" } end,
  ...
  cmd("DwightWorkspaceRemove", function(o)
    ...
    complete = function()
      ...
      return vim.tbl_map(function(r) return r.name end, ws.repos())
    end,
  ...
  cmd("DwightDevDocs", function(o)
    ...
    nargs = "?",
    complete = function() return { "architecture", "api-reference", "getting-started", "--agentic" } end,
  ...
  cmd("DwightDiffToggle", function()
    M.config.diff_preview = not M.config.diff_preview
    vim.notify("[dwight] Diff preview: " .. (M.config.diff_preview and "ON" or "OFF"), vim.log.levels.INFO)
  end, { desc = "Toggle diff preview before applying changes" })

  -- Streaming toggle
  cmd("DwightStreamToggle", function()
    M.config.streaming = not M.config.streaming
    vim.notify("[dwight] Streaming: " .. (M.config.streaming and "ON" or "OFF"), vim.log.levels.INFO)
  end, { desc = "Toggle streaming responses" })

  -- Git context toggle
  cmd("DwightGitToggle", function()
    M.config.git_context = not M.config.git_context
    vim.notify("[dwight] Git context: " .. (M.config.git_context and "ON" or "OFF"), vim.log.levels.INFO)
  ...
  cmd("DwightMinimap", function(o)
    ...
    if content then
      ...
      vim.bo[buf].buftype = "nofile"
      vim.bo[buf].bufhidden = "wipe"
      vim.cmd("split"); vim.api.nvim_win_set_buf(0, buf)
    else
  ...
  cmd("DwightRefactor", function(o)
    ...
    complete = function()
      ...
      if ok then return features.names() end
      return {}
    end,
...
function M.dot_repeat()
  ...
  if mode_def then
    ...
    require_mod("inline").run(prompt_text, selection, M.config, op.mode_name,
      op.model_override, op.think_depth, op.opts)
  else
    -- Freeform: replay original prompt
    require_mod("inline").run(op.prompt_text, selection, M.config, op.mode_name,
      op.model_override, op.think_depth, op.opts)
  end
...
function M.status()
  ...
  elseif backend == "opencode" then
    ...
    model_line = string.format("Backend: api | Provider: %s | Model: %s %s",
      resolved.provider_name, resolved.model_id, key_status)
  end

  local feature_count = 0
  pcall(function() feature_count = #require_mod("features").list() end)

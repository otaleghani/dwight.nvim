...
function M._register_commands()
	...
	cmd("DwightExecute", function()
		vim.notify(
			"[dwight] ⚠️ DwightExecute is deprecated. Use :DwightAgent instead (agentic mode).",
			vim.log.levels.WARN
		)
	...
	cmd("DwightAgentResume", function()
		vim.notify(
			"[dwight] ⚠️ DwightAgentResume is deprecated. Use :DwightAutoResume for multi-task sessions.",
			vim.log.levels.WARN
		)
		...
		elseif o.args == "stats" then
			...
			for i, l in ipairs(lessons) do
				...
				if (l.reinforced or 1) > 1 then
					meta_parts[#meta_parts + 1] = "×" .. l.reinforced
				end
				...
				if l.timestamp then
					...
					if age > 30 * 86400 then
						meta_parts[#meta_parts + 1] = "⚠️ stale"
					end
				end

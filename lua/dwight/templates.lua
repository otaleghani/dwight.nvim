      ...
      while true do
        ...
        if (ftype == "file" or ftype == "link") and name:match("%.txt$") then
          ...
          if f then
            ...
            all[#all + 1] = { key = key, name = key, icon = "📝", prompt = vim.trim(content), builtin = false }
          end
        end
      end
    end
...
function M.save(prompt_text)
  ...
  vim.ui.input({ prompt = "Template name: " }, function(name)
    ...
    if f then
      f:write(prompt_text or ""); f:close()
...
function M.pick(callback)
  ...
  if #templates == 0 then
    vim.notify("[dwight] No templates available.", vim.log.levels.INFO); return
  end
  ...
  if has_tel then
    ...
    pickers.new({}, {
      ...
      attach_mappings = function(pb)
        actions.select_default:replace(function()
          ...
          actions.close(pb)
          if sel and callback then callback(sel.value.prompt) end
        end)
        return true
      end,
    }):find()
    ...
    local items = {}
    for i, t in ipairs(templates) do items[i] = i .. ". " .. t.icon .. " " .. t.name end
    vim.ui.input({ prompt = table.concat(items, " | ") .. " > " }, function(c)
      if not c then return end
      local idx = tonumber(c)
      if idx and templates[idx] and callback then callback(templates[idx].prompt) end
    end)
  end
end

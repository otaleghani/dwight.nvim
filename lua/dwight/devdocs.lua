...
local function build_doc_context()
  ...
  if #names > 0 then
    ...
    for _, name in ipairs(names) do
      ...
      if xml then parts[#parts + 1] = xml end
    end
  end
...
function M.generate(doc_name)
  ...
  -- Generate all docs
  vim.notify(string.format("[dwight] 📖 Generating %d developer docs from %d features…",
    #DOC_TYPES, #names), vim.log.levels.INFO)
  ...
  for _, dt in ipairs(DOC_TYPES) do
    generate_doc(dt, context, function(content, err)
      ...
      if pending == 0 then
        vim.notify(string.format("[dwight] ✅ Generated %d/%d docs in .dwight/dev/",
          generated, #DOC_TYPES), vim.log.levels.INFO)
        if generated > 0 then
...
function M.browse()
  ...
  if handle then
    while true do
      ...
      if ftype == "file" and name:match("%.md$") then files[#files + 1] = name end
    end
  end
  table.sort(files)
...
- Do NOT create extra files beyond the doc targets
    ...
    #existing > 0
      and ("Read these existing docs for style: " .. table.concat(existing, ", "))
      or "No existing docs — establish a clear, concise technical style.",
    devdocs_dir())

  vim.notify(string.format(
    "[dwight] 🤖 Agentic dev docs: %d doc(s) from %d features…",
    #target_docs, #names), vim.log.levels.INFO)
      ...
      vim.schedule(function()
        if success then
          ...
          vim.notify(string.format(
            ...
            generated, #target_docs), vim.log.levels.INFO)
          if vim.fn.filereadable(devdocs_dir() .. "/architecture.md") == 1 then
            vim.cmd("edit " .. vim.fn.fnameescape(devdocs_dir() .. "/architecture.md"))
          end
        else
          vim.notify("[dwight] 🤖 Dev docs had errors. Check :DwightAgentStatus.",
            vim.log.levels.WARN)
        end
      end)
    end,

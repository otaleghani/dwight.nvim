...
function M._run_api(prompt, callback)
  ...
  local ok_enc, payload = pcall(function()
    if format == "anthropic" then
      return vim.json.encode({ model = model_id, max_tokens = max_tokens,
    ...
    vim.schedule(function()
      ...
      if code ~= 0 then
        vim.notify(string.format(
          "[dwight] curl failed (exit %d). Response: %s",
          code, raw_json:sub(1, 200)), vim.log.levels.WARN)
        callback("", code)
        return
      end
      ...
      if resp.error then
        local msg = type(resp.error) == "table"
          and (resp.error.message or vim.inspect(resp.error))
        ...
        if msg:lower():match("utf") or msg:lower():match("not valid json") then
          ...
          vim.notify(string.format(
            ...
            msg:sub(1, 200), #payload, table.concat(byte_str, " ")), vim.log.levels.ERROR)
        else
          ...
          for _, cand in ipairs(resp.candidates) do
            if cand.content and cand.content.parts then
              for _, part in ipairs(cand.content.parts) do
                if part.text then parts[#parts + 1] = part.text end
              end
            end
          end
        end
...
function M._run_claude_code(prompt, callback)
  ...
  if spawn_env then spawn_opts.env = spawn_env end
    ...
    vim.schedule(function()
      ...
      if code ~= 0 then
        if err_output:match("not found") or err_output:match("command not found") then
          vim.notify("[dwight] Claude Code not found. Install: npm i -g @anthropic-ai/claude-code",
            vim.log.levels.ERROR)
        elseif err_output:match("auth") or err_output:match("login") then
          vim.notify("[dwight] Claude Code not authenticated. Run 'claude' in terminal to log in.",
            vim.log.levels.ERROR)
        else
  ...
  if not handle then
    vim.notify("[dwight] Failed to spawn claude. Is it installed? npm i -g @anthropic-ai/claude-code",
      vim.log.levels.ERROR)
    callback("", 1)
    return
  end

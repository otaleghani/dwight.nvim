...
local function get_recent_jobs(max_jobs)
	...
	for i = #jobs, math.max(1, #jobs - max_jobs + 1), -1 do
		...
		if j and j.status == "success" then
			recent[#recent + 1] =
				string.format("- Mode: %s | Instructions: %s", j.mode or "?", (j.prompt or ""):sub(1, 200))
		end
	end
...
local function looks_like_commit_title(line)
	...
	if
		trimmed:match("^I ")
		or trimmed:match("^Now ")
		or trimmed:match("^Let me")
		or trimmed:match("^Here")
		or trimmed:match("^So ")
		or trimmed:match("^The ")
		or trimmed:match("^This is")
		or trimmed:match("^Based on")
		or trimmed:match("^OK")
		or trimmed:match("^Alright")
		or trimmed:match("^Sure")
		or trimmed:match("^Looking")
	then
		return false
	end
...
local function strip_preamble(raw)
	...
	if looks_like_commit_title(lines[1]) then
		return msg
	end
	...
	for i, line in ipairs(lines) do
		local trimmed = vim.trim(line)
		if trimmed ~= "" and #trimmed <= 72 and not trimmed:match("%.$") and not trimmed:match(" I ") then
			local rest = {}
			for j = i, #lines do
				rest[#rest + 1] = lines[j]
			end
...
Configurable per-route limits via gateway.yml.
		...
		files,
		diff,
		jobs ~= "" and jobs or "(no recent AI changes)"
	)

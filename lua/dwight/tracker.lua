...
local function get_pricing(model)
  if not model then return M.PRICING.default end
  local m = model:lower()
  for pattern, price in pairs(M.PRICING) do
    if pattern ~= "default" and m:find(pattern, 1, true) then return price end
  end
...
function M.record(mode_name, chars_sent, chars_received, extra)
  ...
  if extra.feature then
    ...
    if extra.status == "success" then sf.success = sf.success + 1
    elseif extra.status == "error" or extra.status == "parse_fail" then sf.failure = sf.failure + 1
    end
    if extra.duration then sf.duration = sf.duration + extra.duration end
  end
  ...
  if extra.feature then
    ...
    if not lt.by_feature[extra.feature] then
      lt.by_feature[extra.feature] = {
        invocations = 0, cost = 0, success = 0, failure = 0,
    ...
    if extra.status == "success" then lf.success = (lf.success or 0) + 1
    elseif extra.status == "error" or extra.status == "parse_fail" then lf.failure = (lf.failure or 0) + 1
    end
    if extra.duration then lf.duration = (lf.duration or 0) + extra.duration end
  ...
  if extra.status then
    ...
    if not data.daily_status[today] then data.daily_status[today] = { success = 0, failure = 0 } end
...
function M.set_model(model) M._session.model = model end
...
function M.get_model()
  ...
  if M._session.model then return M._session.model end
  -- Try to get from providers module (reflects DwightSwitch)
  local ok, providers = pcall(require, "dwight.providers")
  if ok and providers._active_model then return providers._active_model end
...
local function tk(chars) return math.floor(chars / 4) end
...
local function fmt_tkn(chars)
  local t = tk(chars)
  if t >= 1000000 then return string.format("%.1fM", t / 1000000) end
  if t >= 1000 then return string.format("%.1fK", t / 1000) end
  return tostring(t)
end
...
function M.show()
  ...
  table.sort(model_list, function(a, b) return a.stats.cost > b.stats.cost end)

  if #model_list > 0 then
    lines[#lines + 1] = "── Model Breakdown (lifetime) ──────────────────"
    lines[#lines + 1] = string.format("  %-20s %-10s %5s %8s %8s %8s",
      "Model", "Backend", "Calls", "Tok In", "Tok Out", "Cost")
    ...
    for _, entry in ipairs(model_list) do
      ...
      if p.name and p.name ~= "Unknown" then display = p.name end
      if #display > 20 then display = display:sub(1, 17) .. "…" end
      -- Determine backend for this model
      local model_backend = st.backend or backend
      lines[#lines + 1] = string.format("  %-20s %-10s %5s %8s %8s %8s",
        display, model_backend, fmt_num(st.invocations), fmt_tkn(st.chars_sent),
        fmt_tkn(st.chars_received), fmt_cost(st.cost))
    end
    lines[#lines + 1] = "  " .. string.rep("─", 65)
    lines[#lines + 1] = string.format("  %-20s %-10s %5s %8s %8s %8s",
      "TOTAL", "", fmt_num(lt.invocations), fmt_tkn(lt.chars_sent),
      fmt_tkn(lt.chars_received), fmt_cost(lt.cost))
    lines[#lines + 1] = ""
  end

  -- Session summary
  lines[#lines + 1] = "── This Session ────────────────────────────────"
  lines[#lines + 1] = string.format("  Invocations: %s  |  Tokens: ~%s in, ~%s out  |  Cost: %s",
    fmt_num(s.invocations), fmt_tkn(s.chars_sent), fmt_tkn(s.chars_received), fmt_cost(s.cost or 0))

  -- Session per-model if multiple
  local s_models = {}
  for model, stats in pairs(s.by_model) do s_models[#s_models + 1] = { model = model, stats = stats } end
  if #s_models > 1 then
    table.sort(s_models, function(a, b) return a.stats.cost > b.stats.cost end)
    for _, entry in ipairs(s_models) do
      local display = entry.model
      local p = get_pricing(entry.model)
      if p.name and p.name ~= "Unknown" then display = p.name end
      lines[#lines + 1] = string.format("    %s: %s calls, %s",
        display, fmt_num(entry.stats.invocations), fmt_cost(entry.stats.cost))
    end
  end
  lines[#lines + 1] = ""
  ...
  if data.daily_cost then
    local days = {}
    for day, cost in pairs(data.daily_cost) do days[#days + 1] = { day, cost } end
    table.sort(days, function(a, b) return a[1] > b[1] end)
    if #days > 0 then
      lines[#lines + 1] = "── Daily Cost ──────────────────────────────────"
  ...
  table.sort(price_list, function(a, b) return a.input < b.input end)
  for _, p in ipairs(price_list) do
    lines[#lines + 1] = string.format("  %-24s %7s %8s",
      p.name, "$" .. string.format("%.2f", p.input), "$" .. string.format("%.2f", p.output))
  end

...
local function extract_signatures(content, lang, _path)
  ...
  local function add(line)
    line = vim.trim(line)
    if line == "" or seen[line] then return end
    if #lines >= MAX_LINES_PER_FILE then return end
    seen[line] = true
    lines[#lines + 1] = line
  end
  ...
  if lang == "go" then
    ...
    if pkg then add("package " .. pkg) end
    for decl in content:gmatch("\ntype%s+(%S+%s+%S+)") do add("type " .. decl) end
    for sig in content:gmatch("\nfunc%s+([^\n{]+)") do
      local name = sig:match("^%(.-%)%s+([A-Z]%w*)") or sig:match("^([A-Z]%w*)")
      if name then add("func " .. sig:gsub("%s*{%s*$", "")) end
    end
  ...
  elseif lang == "typescript" or lang == "javascript" then
    for line in content:gmatch("\nexport%s+([^\n]+)") do
      ...
      if #decl < 120 then add("export " .. decl) end
    end
    for kind in content:gmatch("\n(interface%s+%S+)") do add(kind) end
    for kind in content:gmatch("\ntype%s+(%S+%s*=)") do add("type " .. kind .. " ...") end

  elseif lang == "lua" then
    for sig in content:gmatch("\nfunction%s+(M%.%S+%s*%([^)]*%))") do add(sig) end
  ...
  elseif lang == "python" then
    ...
    for imp in content:gmatch("\nfrom%s+(%S+)%s+import") do
      if import_count < 2 then add("from " .. imp .. " import ..."); import_count = import_count + 1 end
    end
    for cls in content:gmatch("\nclass%s+([^:(\n]+)") do add("class " .. vim.trim(cls)) end
    for sig in content:gmatch("\ndef%s+([^\n:]+)") do
      local name = sig:match("^(%S+)%(")
      if name and not name:match("^_[a-z]") then add("def " .. sig) end
    end
  ...
  elseif lang == "rust" then
    ...
    for kind in content:gmatch("\npub%s+(struct%s+%S+)") do add("pub " .. kind) end
    for kind in content:gmatch("\npub%s+(enum%s+%S+)") do add("pub " .. kind) end
    for kind in content:gmatch("\npub%s+(trait%s+%S+)") do add("pub " .. kind) end
  ...
  elseif lang == "ruby" then
    for cls in content:gmatch("\nclass%s+([^\n<]+)") do add("class " .. vim.trim(cls)) end
    for mod in content:gmatch("\nmodule%s+(%S+)") do add("module " .. mod) end
    for sig in content:gmatch("\ndef%s+([^\n]+)") do
      local name = sig:match("^(%S+)")
      if name and not name:match("^_") then add("def " .. sig) end
    end

  elseif lang == "java" or lang == "kotlin" or lang == "csharp" then
    for cls in content:gmatch("\npublic%s+class%s+(%S+)") do add("public class " .. cls) end
    for sig in content:gmatch("\npublic%s+[%w<>%[%]]+%s+(%S+%s*%([^)]*%))") do add(sig) end

  elseif lang == "swift" then
    for sig in content:gmatch("\nfunc%s+([^\n{]+)") do add("func " .. sig:gsub("%s*{%s*$", "")) end
    for cls in content:gmatch("\nclass%s+(%S+)") do add("class " .. cls) end
    for s in content:gmatch("\nstruct%s+(%S+)") do add("struct " .. s) end
    for p in content:gmatch("\nprotocol%s+(%S+)") do add("protocol " .. p) end
  ...
  if #lines == 0 then
    ...
    if first_comment then add(vim.trim(first_comment)) end
  end
...
local function scan_source_files()
  ...
  local function walk(dir, prefix)
    ...
    while true do
      ...
      if name:sub(1, 1) == "." and name ~= ".." then goto continue end
      if SKIP_DIRS[name] then goto continue end
      ...
      elseif ftype == "file" then
        ...
        if SKIP_EXT[ext] then goto continue end
        ...
        if lang then
          ...
          if size < 100000 then
            files[#files + 1] = {
              path = rel, full = full, mtime = mtime,
              ext = ext, lang = lang, size = size,
            }
          end
        end
      end
...
local function prioritize_files(files)
  ...
  local function is_entry(rel)
    return rel:match("^main%.") or rel:match("^index%.") or rel:match("^app%.")
      or rel:match("^server%.") or rel:match("^src/main") or rel:match("^src/index")
      or rel:match("^src/app") or rel:match("^cmd/") or rel:match("^lib/")
  end
  ...
  table.sort(files, function(a, b)
    ...
    if a_e ~= b_e then return a_e > b_e end
...
function M.build(opts)
  ...
  pcall(function()
    ...
    if vim.v.shell_error == 0 then git_head = vim.trim(out) end
  end)
  ...
  if not opts.silent then
    vim.notify(string.format(
      ...
      file_count, extracted, reused, #source_files),
      vim.log.levels.INFO)
  end
...
function M.is_stale()
  ...
  pcall(function()
    local out = vim.fn.system("git rev-parse --short HEAD 2>/dev/null")
    if vim.v.shell_error == 0 then current_head = vim.trim(out) end
  end)
...
function M.format_for_prompt()
  local digest = M.get()
  if not digest or not digest.files then return nil end
  ...
  if total_size == 0 then return nil end
...
function M.status()
  ...
  for _, entry in pairs(cached.files or {}) do
    ...
    if entry.summary then
      for _ in entry.summary:gmatch("\n") do total_summary_lines = total_summary_lines + 1 end
  ...
  vim.notify(string.format(
    ...
    file_count, cached.total_source_files or 0,
    total_summary_lines, prompt_size,

...
function M.update_indicator_label(job_id, label)
  local ind = M._job_indicators[job_id]
  if ind then ind.custom_label = label end
end
...
function M.parse_tokens(text)
  ...
  for feat in text:gmatch("%$([%w_%-%.]+)") do features[#features + 1] = feat end
  for lib in text:gmatch("%%([%w_%-%.]+)") do libs[#libs + 1] = lib end
  for ref in text:gmatch("&([%w_%-%./:]+)") do mcp_refs[#mcp_refs + 1] = ref end
  ...
  for mtype, mval in text:gmatch("!(%w+):([%w_%-%./:]+)") do
    mtype = mtype:lower()
    if mtype == "mode" then mode = mval
    elseif mtype == "model" then model_override = mval
    elseif mtype == "skill" then skills[#skills + 1] = mval
    elseif mtype == "symbol" then symbols[#symbols + 1] = mval
    elseif mtype == "feature" then features[#features + 1] = mval
    elseif mtype == "lib" then libs[#libs + 1] = mval
    elseif mtype == "file" then files[#files + 1] = mval
    elseif mtype == "depth" then
      think_depth = math.min(math.max(tonumber(mval) or 1, 1), 4)
    elseif mtype == "audit" then audit_model = mval
    end
  end
  ...
  if text:match("!%w+:[%w_%-%./:]+") then
    ...
    if bare and not bare:match("^%w+:") then model_override = bare end
  end
...
local function highlight_prompt_buf(buf, editable_end)
  ...
  local feat_set = {}
  pcall(function()
    for _, n in ipairs(require("dwight.features").names()) do feat_set[n] = true end
  end)
  local lib_set = {}
  pcall(function()
    for _, n in ipairs(require("dwight.libs").names()) do lib_set[n] = true end
  end)
  ...
  elseif trigger == "+" then
    ...
    pcall(function()
      ...
      for _, f in ipairs(ts.project_files(typed)) do
        ...
        if #items >= 30 then break end  -- limit for performance
      end
    end)
  elseif trigger == "~" then
    -- ~audit completion
...
function M.open_prompt(selection, _preset_mode)
  ...
  local height = EDITABLE_LINES + 9  -- editable + divider + help
  ...
  pcall(function()
    ...
    elseif backend == "opencode" then
      ...
      if #short > 20 then short = short:sub(1, 17) .. "…" end
      model_display = resolved.provider_name .. "/" .. short
    end
  end)

  local content = {}
  for _ = 1, EDITABLE_LINES do content[#content + 1] = "" end
  content[#content + 1] = "─── " .. file_info .. " · " .. model_display .. " ───"
  content[#content + 1] = "  /mode   fix · test · stub · code · implement · lint · macro · plan …"
  content[#content + 1] = "  @skill  $feature  #symbol  %lib  &mcp:  +file  ~audit:model"
  content[#content + 1] = "  !type:value  unified: !mode: !model: !skill: !file: !feature: …"
  content[#content + 1] = "  ^N  think depth    <CR> submit   <Tab> complete   q quit"
  content[#content + 1] = ""
      ...
      for _, fpath in ipairs(parsed.files or {}) do
        pcall(function()
          ...
          if vim.fn.isdirectory(full) == 1 then
            ...
            if content then resolved_files[#resolved_files + 1] = { name = fpath, content = content } end
          else
        ...
        if parsed.mode then
          ...
          if mode then
            ...
            local prompt_text = require("dwight.prompt").build(
              mode, selection, ctx, parsed.clean_text,
              skill_paths, resolved_symbols, resolved_features,
              parsed.think_depth, resolved_libs, resolved_mcp,
              resolved_files)
            require("dwight.inline").run(
              prompt_text, selection, cfg, parsed.mode,
              parsed.model_override, parsed.think_depth,
              {
          ...
          local prompt_text = require("dwight.prompt").build_freeform(
            parsed.clean_text, selection, skill_paths,
            resolved_symbols, resolved_features,
            parsed.think_depth, resolved_libs, resolved_mcp,
            resolved_files)
          require("dwight.inline").run(
            prompt_text, selection, cfg, "custom",
            parsed.model_override, parsed.think_depth,
            { audit_model = parsed.audit_model })
        end
      end)

...
T.test("save and load lessons", function()
	agent._save_lessons({
		...
		{
			keywords = { "go", "mock" },
			text = "Mock all interface methods",
			timestamp = os.time(),
			reinforced = 2,
			use_count = 3,
		},
...
T.test("merge similar lesson instead of duplicating", function()
	agent._save_lessons({
		{
			keywords = { "go", "test", "mock" },
			text = "Old mock advice",

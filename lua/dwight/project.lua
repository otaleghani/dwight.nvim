...
function M._copy_builtin_skills()
	...
	if not builtin or vim.fn.isdirectory(builtin) ~= 1 then
		return {}
	end
...
function M.install_builtins()
	...
	local copied = M._copy_builtin_skills()
	if #copied > 0 then
		vim.notify("📎 [dwight] Installed: " .. table.concat(
			vim.tbl_map(function(n)
				return "@" .. n
			end, copied),
			", "
...
function M.read_scope()
	...
	if jit_ok and jit_context then
		return jit_context
	end
	...
	if trimmed == "" or trimmed == "# Project Scope" then
		return nil
	end
	return content
end

...
function M.build(mode, selection, ctx, extra_instructions,
                 skill_paths, resolved_symbols, resolved_features,
                 think_depth, resolved_libs, resolved_mcp,
                 resolved_files)
  ...
  local code_section = string.format(
    ...
    ctx.language, vim.fn.fnamemodify(selection.filepath or "", ":."),
  ...
  pcall(function()
    ...
    if #all_names > 0 and #resolved_features == 0 then
      ...
      if index then
        optional[#optional + 1] = budget.section(P.ARCHITECTURE - 5, "feature_index",
          "\n<feature_index>\n" .. index .. "\n</feature_index>")
      end
    end
  end)
  ...
  if not mode.skip_rag then
    pcall(function()
      ...
      if rag_ctx then
        optional[#optional + 1] = budget.section(P.SYMBOLS - 5, "rag",
          "\n<codebase_search>\n" .. rag_ctx .. "\n</codebase_search>")
      end
    end)
  end
  ...
  if #all_skill_paths > 0 then
    ...
    for _, path in ipairs(all_skill_paths) do
      ...
      if content then skill_parts[#skill_parts + 1] = content end
    end
  ...
  if #resolved_symbols > 0 then
    ...
    for _, sym in ipairs(resolved_symbols) do
      sym_parts[#sym_parts + 1] = string.format('  <symbol name="%s" kind="%s" file="%s" line="%d" />',
        sym.name, sym.kind, vim.fn.fnamemodify(sym.filepath, ":."), sym.line)
      if sym.text then sym_parts[#sym_parts + 1] = sym.text end
    end
  ...
  if mode.inject_run_output then
    pcall(function()
      ...
      if tdd_ctx then
        optional[#optional + 1] = budget.section(P.TEST_CONTEXT, "tdd",
          "\n<test_context>\n" .. tdd_ctx .. "\n</test_context>")
      end
    end)
  end
  ...
  if run_ctx and mode.inject_run_output then
    optional[#optional + 1] = budget.section(P.RUN_OUTPUT, "run_output",
      "\n<build_output>\n" .. run_ctx .. "\n</build_output>")
  end
  ...
  if cfg.git_context ~= false and not mode.skip_git then
    pcall(function()
      ...
      if git_ctx then
        optional[#optional + 1] = budget.section(P.GIT_CONTEXT, "git",
          "\n<git_context>\n" .. git_ctx .. "\n</git_context>")
      end
    end)
  end
  ...
  pcall(function()
    ...
    if filepath and filepath ~= "" then
      ...
      if minimap and minimap ~= "" then
        optional[#optional + 1] = budget.section(P.LSP_DIAGNOSTICS, "file_structure",
          string.format('\n<file_structure path="%s">\n%s\n</file_structure>',
            vim.fn.fnamemodify(filepath, ":."), minimap))
      end
    end
  end)
  ...
  if ctx.diagnostics and #ctx.diagnostics > 0 then
    ...
    for _, d in ipairs(ctx.diagnostics) do
      diag_parts[#diag_parts + 1] = string.format('  <diagnostic line="%s" severity="%s">%s</diagnostic>',
        d.line or "?", d.severity or "HINT", d.message or "")
    end
    diag_parts[#diag_parts + 1] = "</diagnostics>"
    optional[#optional + 1] = budget.section(P.LSP_DIAGNOSTICS + 2, "diagnostics",
  ...
  local fixed_cost = 0
  for _, p in ipairs(parts) do fixed_cost = fixed_cost + math.ceil(#p / 4) end
...
</rules>]], ctx.language, cs.line, scope_reminder)
  else
    parts[#parts + 1] = string.format([[
...
5. If unchanged, return original code in a code block.%s
      ...
      selection.start_line, selection.end_line, cs.line, scope_reminder)
  end

  return table.concat(parts, "\n")
end

function M.build_freeform(user_text, selection, skill_paths,
  ...
  return M.build(custom_mode, selection, ctx, nil,
                 skill_paths, resolved_symbols, resolved_features,
                 think_depth, resolved_libs, resolved_mcp,
                 resolved_files)
end

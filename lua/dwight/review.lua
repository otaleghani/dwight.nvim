...
]], language, original_code, language, proposed_code,
    user_instructions or "", language)
end
...
function M.review(original_code, proposed_code, language, user_instructions, audit_model, callback)
  ...
  if format == "anthropic" then
    payload = vim.json.encode({
      model = resolved.model_id, max_tokens = cfg.max_tokens or 4096,
  ...
  if format == "gemini" then endpoint = endpoint:gsub("{model}", resolved.model_id) end
  local url = (resolved.provider.base_url or "") .. endpoint

  local curl_args = {
    "-sS", "--max-time", tostring(math.floor((cfg.timeout or 120000) / 1000)),
    "-X", "POST", url, "-H", "Content-Type: application/json",
  }
  if resolved.provider.auth_header then
    table.insert(curl_args, "-H")
    table.insert(curl_args, resolved.provider.auth_header .. ": " ..
      (resolved.provider.auth_prefix or "") .. api_key)
  end
  if resolved.provider.auth_query then curl_args[6] = url .. "?key=" .. api_key end
  ...
  handle = uv.spawn("curl", {
    ...
    vim.schedule(function()
      if code ~= 0 then callback(nil, "Audit request failed"); return end

      local raw = table.concat(chunks, "")
      local ok, resp = pcall(vim.json.decode, raw)
      if not ok then callback(nil, "Invalid JSON from audit"); return end
      ...
      elseif format == "gemini" then
        ...
        if resp.choices then
          for _, c in ipairs(resp.choices) do
            if c.message and c.message.content then text = text .. c.message.content end
          end
        end
      end
      ...
      if not reviewed then
        ...
        while true do
          ...
          if not block_end then break end
        ...
        if #blocks > 0 then
          ...
          for _, b in ipairs(blocks) do
            if #b > #reviewed then reviewed = b end
          end
        end
      end
      ...
      if not reviewed and text:match("%[REVIEW") then
        ...
        if #stripped > 0 then reviewed = stripped end
      end
  ...
  if not handle then pcall(os.remove, payload_file); callback(nil, "spawn failed"); return end
  stdout:read_start(function(e, d) if not e and d then chunks[#chunks + 1] = d end end)
  stderr:read_start(function(e, d) if not e and d then err_chunks[#err_chunks + 1] = d end end)
end

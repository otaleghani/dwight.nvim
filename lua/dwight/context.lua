...
function M._scan_node(cwd)
  ...
  if type(pkg.scripts) == "table" then
    ...
    table.sort(info.scripts, function(a, b) return a.name < b.name end)
  end
  ...
  if type(pkg.dependencies) == "table" then
    ...
    table.sort(info.deps, function(a, b) return a.name < b.name end)
  end
  ...
  if type(pkg.devDependencies) == "table" then
    ...
    table.sort(info.dev_deps, function(a, b) return a.name < b.name end)
  end
...
function M.scan()
  ...
  -- Try each scanner
  local info = M._scan_go(cwd)
    or M._scan_node(cwd)
    or M._scan_rust(cwd)
    or M._scan_python(cwd)
...
function M.build_xml()
  ...
  if info.type == "go" then
    ...
    if info.module then parts[#parts + 1] = string.format("  <module>%s</module>", info.module) end
    if info.go_version then parts[#parts + 1] = string.format("  <go_version>%s</go_version>", info.go_version) end
  ...
  if info.type == "node" then
    ...
    if info.name then parts[#parts + 1] = string.format("  <name>%s</name>", info.name) end
  ...
  if info.type == "rust" then
    ...
    if info.name then parts[#parts + 1] = string.format("  <name>%s</name>", info.name) end
    if info.edition then parts[#parts + 1] = string.format("  <edition>%s</edition>", info.edition) end
...
function M.install_cmd()
  local info = M.scan()
  if not info then return nil end
  if info.type == "go" then return "go get {package}" end
  if info.type == "rust" then return "cargo add {package}" end
  if info.type == "python" then
    if info.pkg_manager == "poetry" then return "poetry add {package}" end
    if info.pkg_manager == "uv" then return "uv add {package}" end
    return "pip install {package}"
  end
  if info.type == "node" then
    if info.pkg_manager == "yarn" then return "yarn add {package}" end
    if info.pkg_manager == "pnpm" then return "pnpm add {package}" end
    if info.pkg_manager == "bun" then return "bun add {package}" end
...
function M.test_cmd()
  local info = M.scan()
  if not info then return nil end
  if info.type == "go" then return "go test ./..." end
  if info.type == "rust" then return "cargo test" end
  if info.type == "python" then return "pytest" end
  if info.type == "node" then

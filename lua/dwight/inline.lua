...
local function get_config()  return require("dwight").config end
local function get_dwight()  return require("dwight") end
...
local function extract_code_block(raw)
  ...
  local best = blocks[1]
  for i = 2, #blocks do if #blocks[i] > #best then best = blocks[i] end end
  return best
end
...
function M._pre_apply_syntax_check(code, filetype, filepath)
  if not code or code == "" then return true, nil end
    ...
    for _, change in ipairs(files) do
      if change.content and change.path and change.action ~= "delete" then
        ...
        if not change.start_line then
          local full_path = change.path:match("^/") and change.path
            or (vim.fn.getcwd() .. "/" .. change.path)
          ...
          if not ok then
            vim.notify(string.format(
              ...
              job_id or 0, change.path), vim.log.levels.WARN)
            local log = require("dwight.log")
            log.finish(job_id, "pre_check_fail", "", nil,
...
local function new_job_id() return require("dwight.log")._next_id() end
...
local function handle_response(job_id, raw_output, err_output, exit_code, selection, mode_name, prompt_text)
  ...
  if job and (job.is_prose or job.is_plan) then
    ...
    if get_config().diff_preview then
      require("dwight.diff").show(bufnr, cur_start, cur_end, content,
        function()
          ...
          pcall(function() require("dwight.tracker").record(mode_name or "prose", #prompt_text, #raw_output) end)
  ...
  if job and job.audit_model then
    ...
    if audit_model_name == true then audit_model_name = nil end  -- use alternate model

    pcall(function()
      require("dwight.review").review(
        selection.text, parsed_code, selection.filetype or "code",
        prompt_text, audit_model_name,
        function(reviewed_code, err)
          ...
          M._apply_parsed(job_id, bufnr, cur_start, cur_end, parsed_code, new_lines,
            raw_output, selection, mode_name, prompt_text)
        end)
    end)
...
function M._apply_parsed(job_id, bufnr, cur_start, cur_end, parsed_code, new_lines,
                          raw_output, selection, mode_name, prompt_text)
  local log = require("dwight.log")
  local cfg = get_config()
  ...
  if not cfg.diff_preview and cfg.pre_apply_check ~= false then
    ...
    if cur_start and cur_end and api.nvim_buf_is_valid(bufnr) then
      ...
      local before = {}
      for i = 1, cur_start - 1 do before[#before + 1] = buf_lines[i] end
      local after = {}
      for i = cur_end + 1, #buf_lines do after[#after + 1] = buf_lines[i] end
    ...
    if not check_ok then
      log.finish(job_id, "pre_check_fail", raw_output, parsed_code,
        "Pre-apply check failed: " .. (check_err or "unknown"))
      vim.notify(string.format("[dwight] ⚠️ Job #%d: pre-apply check failed, rejecting output.", job_id),
        vim.log.levels.WARN)
      return
    end
  end
  ...
  local cfg = get_config()
  if cfg.diff_preview then
    require("dwight.diff").show(bufnr, cur_start, cur_end, parsed_code,
      function()  -- on_accept
        M._replace_selection_atomic(bufnr, cur_start, cur_end, parsed_code)
        adjust_other_jobs(job_id, bufnr, cur_start, cur_end, #new_lines)
        log.finish(job_id, "success", raw_output, parsed_code, nil)
        pcall(function() require("dwight.tracker").record(mode_name or "custom", #prompt_text, #raw_output) end)
        vim.notify("[dwight] ✅ Job #" .. job_id .. " accepted.", vim.log.levels.INFO)
      end,
      function(reason)  -- on_reject
...
local function build_payload(format, prompt_text, model_id, max_tokens, stream)
  if format == "anthropic" then
    local body = { model = model_id, max_tokens = max_tokens,
...
local function parse_sse_chunk(format, line)
  ...
  if data == "[DONE]" then return nil end
  ...
  if format == "anthropic" then
    ...
    if event.choices and event.choices[1] then
      ...
      if delta and delta.content then return delta.content end
    end
  end
  return nil
end
...
local function extract_text(format, resp)
  ...
  if format == "anthropic" then
    if resp.content then
      for _, b in ipairs(resp.content) do if b.type == "text" then parts[#parts + 1] = b.text end end
    end
    if resp.usage then
  ...
  elseif format == "gemini" then
    if resp.candidates then
      for _, c in ipairs(resp.candidates) do
        if c.content and c.content.parts then
          for _, p in ipairs(c.content.parts) do if p.text then parts[#parts + 1] = p.text end end
        end
      end
    end
  else
    if resp.choices then
      for _, c in ipairs(resp.choices) do
        if c.message and c.message.content then parts[#parts + 1] = c.message.content end
      end
    end
    if resp.usage then
      parts[#parts + 1] = string.format("\n\n<!-- tokens: %d in, %d out -->",
...
local function api_call(prompt_text, model_override, cfg, callback, opts)
  ...
  if not provider then callback(nil, "No provider configured"); return end

  local api_key = providers.get_api_key(provider)
  if not api_key then callback(nil, "No API key for " .. resolved.provider_name); return end
  ...
  handle = uv.spawn("curl", {
    args = curl_args, stdio = { nil, stdout, stderr },
  }, function(code)
    ...
    vim.schedule(function()
      ...
      if code ~= 0 then callback(nil, "curl failed: " .. err_output); return end
      ...
      if not ok then callback(nil, "Invalid JSON"); return end
      if resp.error then
        local msg = type(resp.error) == "table" and (resp.error.message or vim.inspect(resp.error)) or tostring(resp.error)
      ...
      while true do
        ...
        if text then
          ...
          if stream_chars % 200 < #text and opts.on_progress then
            vim.schedule(function() opts.on_progress(stream_chars) end)
          end
        end
      end
    end)
...
local function run_api(prompt_text, selection, cfg, mode_name, job_id, model_override, think_depth)
  ...
  if think_depth <= 2 then
    ...
    if djob and handle then djob.handle = handle end
    return
  end
...
local function run_opencode(prompt_text, selection, cfg, mode_name, job_id)
  ...
  if not f then
    ...
    ui.clear_indicators(job_id); dwight._active_jobs[job_id] = nil; return
  end
  f:write(prompt_text); f:close()

  local extra_flags = {}
  if cfg.model then extra_flags[#extra_flags + 1] = "--model"; extra_flags[#extra_flags + 1] = cfg.model end
  for _, flag in ipairs(cfg.opencode_flags or {}) do extra_flags[#extra_flags + 1] = flag end
  ...
  for _, flag in ipairs(extra_flags) do wf:write(string.format(' %s', vim.fn.shellescape(flag))) end
  ...
  handle = uv.spawn("sh", {
    ...
    if handle then handle:close() end
    pcall(os.remove, prompt_file); pcall(os.remove, wrapper)
    vim.schedule(function()
      handle_response(job_id, table.concat(stdout_chunks, ""),
        table.concat(stderr_chunks, ""), code, selection, mode_name, prompt_text)
    end)
  end)
  ...
  local djob = dwight._active_jobs[job_id]
  if djob then djob.handle = handle end
...
local function run_claude_code(prompt_text, selection, cfg, mode_name, job_id, model_override)
  ...
  -- Track model
  pcall(function() require("dwight.tracker").set_model(model or "claude-code") end)
  ...
  handle = uv.spawn(claude_bin, {
    args = args, stdio = { nil, stdout, stderr }, cwd = vim.fn.getcwd(),
  }, function(code)
    if stdout then stdout:close() end; if stderr then stderr:close() end
    if handle then handle:close() end
...
function M.run(prompt_text, selection, cfg, mode_name, model_override, think_depth, opts)
  ...
  timer:start(timeout_ms, 0, function()
    ...
    if job then
      ...
      vim.schedule(function()
        ui.clear_indicators(job_id); dwight._active_jobs[job_id] = nil

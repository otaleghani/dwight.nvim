...
function M.pick(subcmd)
	...
	if subcmd and subcmd ~= "" then
		...
		if subcmd == "commit" then
			return M.smart_commit()
		end
		if subcmd == "cleanup" then
			return M.cleanup()
		end
		if subcmd == "conflict" or subcmd == "conflicts" then
			return M.conflicts()
		end
	...
	require("dwight.select").pick(items, {
		...
		if choice:match("Status") then
			M._show_status_buffer()
		elseif choice:match("Smart commit") then
			M.smart_commit()
		elseif choice:match("Stash changes") then
			M.stash()
		elseif choice:match("Pop stash") then
			M.stash_pop()
		elseif choice:match("Push") then
			M.push()
		elseif choice:match("Pull") then
			M.pull()
		elseif choice:match("Cleanup") then
			M.cleanup()
		elseif choice:match("Resolve") then
			M.conflicts()
		elseif choice:match("Switch branch") then
			M._pick_branch()
...
function M._show_status_buffer()
	...
	if #status.staged == 0 and #status.unstaged == 0 and #status.untracked == 0 and #status.conflicts == 0 then
	...
	pcall(function()
		api.nvim_buf_set_name(buf, "dwight://git-status")
	end)

...
local STRUCTURE_NODES = {
  ...
  arrow_function = true, function_item = true, -- Rust
  func_literal = true, function_statement = true,
  -- Classes / types
  class_declaration = true, class_definition = true,
  struct_item = true, enum_item = true, trait_item = true, impl_item = true, -- Rust
  interface_declaration = true, type_alias_declaration = true,
  -- Exports
  export_statement = true, lexical_declaration = true,
}

-- Nodes that are "public API" (exported, pub, etc.)
local EXPORT_INDICATORS = {
  export_statement = true, visibility_modifier = true,
}
...
function M.smart_select(bufnr)
  ...
  if not parser then return M._fallback_select(bufnr, cursor[1]) end

  local tree = parser:parse()[1]
  if not tree then return M._fallback_select(bufnr, cursor[1]) end
  ...
  if not best_node then return M._fallback_select(bufnr, cursor[1]) end
    ...
    if STRUCTURE_NODES[ctype] then
      ...
      if not sig:match("[{:]%s*$") and not sig:match("%)%s*$") and sr + 1 <= er then
        ...
        for i = sr + 2, math.min(sr + 4, er + 1) do
          if lines[i] then
            sig = sig .. "\n" .. lines[i]
            if lines[i]:match("[{:]%s*$") or lines[i]:match("%)") then break end
          end
        end
      end

      local indent = string.rep("  ", depth)
      if doc ~= "" then results[#results + 1] = indent .. doc end
...
function M._minimap_regex(filepath)
  ...
  for line in source:gmatch("[^\n]+") do
    ...
    if trimmed:match("^%s*function ") or trimmed:match("^%s*local function ")
      or trimmed:match("^%s*def ") or trimmed:match("^%s*async def ")
      or trimmed:match("^%s*class ") or trimmed:match("^%s*export ")
      or trimmed:match("^%s*pub fn ") or trimmed:match("^%s*fn ")
      or trimmed:match("^%s*func ") or trimmed:match("^%s*type ")
      or trimmed:match("^%s*interface ") or trimmed:match("^%s*struct ")
      or trimmed:match("^%s*enum ") or trimmed:match("^%s*impl ")
      or trimmed:match("^%s*const .*= function")
      or trimmed:match("^%s*[%w_]+%s*:%s*function") then
...
function M.minimap_dir(dirpath)
  if vim.fn.isdirectory(dirpath) ~= 1 then return nil end
...
function M.project_files(prefix)
  ...
  for _, f in ipairs(files) do
    ...
    if not rel:match("^%.") and not rel:match("/%.") and not rel:match("node_modules")
      and not rel:match("%.git/") and not rel:match("%.png$") and not rel:match("%.jpg$")
      and not rel:match("%.wasm$") and not rel:match("%.o$") then

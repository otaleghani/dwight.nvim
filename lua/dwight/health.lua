...
local function check_git()
  ...
  local code = vim.v.shell_error
  if code ~= 0 then
    return { ok = false, name = "Git repository", detail = "Not inside a git repo. DwightAuto checkpoints require git." }
  end
...
local function check_auto_session()
  ...
  if state.failed then
    return {
      ok = true, name = "Auto session",
      detail = string.format("FAILED at task %d/%d: %s — use :DwightAutoRetry or :DwightAutoResume",
        task_num, total, (state.tasks[task_num] or {}).title or "?"),
      warn = true,
    }
...
function M._run_checks()
  ...
  pcall(function()
    ...
    checks[#checks + 1] = {
      ...
      name = "Features",
      detail = #names > 0
        and (#names .. " feature(s): " .. table.concat(names, ", "):sub(1, 80))
    ...
    checks[#checks + 1] = {
      ok = true,
      name = "Skills",
      detail = #skill_names > 0
        and (#skill_names .. " skill(s)")
  ...
  for _, c in ipairs(checks) do
    if c then results[#results + 1] = c end
  end
...
function M.show()
  ...
  elseif errors == 0 then
    lines[#lines + 1] = "---"
    lines[#lines + 1] = string.format("**%d warning(s)** — Dwight will work but some features may be limited.", warns)
  else
    lines[#lines + 1] = "---"
    lines[#lines + 1] = string.format("**%d error(s), %d warning(s)** — fix errors above for Dwight to work properly.", errors, warns)
  end
  ...
  lines[#lines + 1] = string.format("- CLI timeout: `%ds`",
    (cfg.agentic_opts and cfg.agentic_opts.cli_timeout) or 600)

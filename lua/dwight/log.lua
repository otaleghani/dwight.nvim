...
function M.set_metadata(job_id, meta)
	...
	if meta.backend then
		entry.backend = meta.backend
	end
	if meta.provider_name then
		entry.provider_name = meta.provider_name
	end
	if meta.tokens_in then
		entry.tokens_in = meta.tokens_in
	end
	if meta.tokens_out then
		entry.tokens_out = meta.tokens_out
	end
	if meta.cost then
...
function M._format_detail(e)
	...
	if e.model or e.backend or e.tokens_in then
		lines[#lines + 1] = ""
		if e.backend then
			lines[#lines + 1] = "**Backend:** " .. e.backend
		end
		if e.model then
			lines[#lines + 1] = "**Model:** " .. e.model
		end
		if e.provider_name then
			lines[#lines + 1] = "**Provider:** " .. e.provider_name
		end
	...
	for i = 1, math.min(60, #prompt_lines) do
		lines[#lines + 1] = prompt_lines[i]
	end
	if #prompt_lines > 60 then
		lines[#lines + 1] = "... (" .. (#prompt_lines - 60) .. " more)"
	end
	lines[#lines + 1] = "```"
	lines[#lines + 1] = ""
	...
	if e.raw_response ~= "" then
		...
		for i = 1, math.min(40, #resp_lines) do
			lines[#lines + 1] = resp_lines[i]
		end
		if #resp_lines > 40 then
			lines[#lines + 1] = "... (" .. (#resp_lines - 40) .. " more)"
		end
		lines[#lines + 1] = "```"
	end

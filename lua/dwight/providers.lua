...
local function global_config_dir()
  ...
  if xdg and xdg ~= "" then return xdg .. "/dwight" end
  return os.getenv("HOME") .. "/.config/dwight"
end

local function global_config_path() return global_config_dir() .. "/providers.json" end

local function project_config_path()
  local project = require("dwight.project")
  if project.is_initialized() then return project.dir() .. "/providers.json" end
  return nil
end
...
function M.load()
  ...
  if global then
    ...
    if global.active_provider then M._active_provider = global.active_provider end
    if global.active_model then M._active_model = global.active_model end
  end
  ...
  if project then
    ...
    if project.active_provider then M._active_provider = project.active_provider end
    if project.active_model then M._active_model = project.active_model end
  end
  ...
  if backend == "api" then
    if cfg.provider then M._active_provider = cfg.provider end
    if cfg.model then M._active_model = cfg.model end
  end
  if not M._active_provider then M._active_provider = "anthropic" end
end
...
function M.save_global()
  ...
  for name, p in pairs(M._providers) do
    if not M.presets[name] then data.providers[name] = p end
  end
...
function M.get_provider(name)
  if not next(M._providers) then M.load() end
...
function M.get_api_key(provider)
  ...
  if provider.api_key then return provider.api_key end
...
function M._read_oauth_token()
  ...
  if not data or not data.access_token then return nil end
  ...
  if data.expires_at and os.time() >= data.expires_at then
    ...
    if data.refresh_token then
      ...
      if refreshed then return refreshed.access_token end
    end
    return nil
  end
...
function M._refresh_oauth_token(token_data)
  ...
  if not token_data.refresh_token then return nil end
...
function M._try_auto_import_cc()
  ...
  if not cc_token then return false end
...
function M.auth_max()
    ...
    require("dwight.select").pick(
      ...
      function(choice)
        ...
        if choice == "Import from Claude Code" then
          ...
          M._active_model = M._providers.anthropic_max.models[M._providers.anthropic_max.default_model]
          M.save_global()
          vim.notify("[dwight] ✅ Imported Claude Code token! Switched to anthropic_max provider.",
            vim.log.levels.INFO)
          vim.notify("[dwight] 💡 Your API calls now bill against your Pro/Max subscription.",
            vim.log.levels.INFO)
  ...
  vim.notify("[dwight]   1. Install Claude Code (npm i -g @anthropic-ai/claude-code) and run 'claude' to authenticate",
    vim.log.levels.INFO)
  ...
  for pname, p in pairs(M._providers) do
    if p.models then
      for alias, _ in pairs(p.models) do
        if not seen[alias] then names[#names + 1] = alias; seen[alias] = true end
    ...
    for pname, p in pairs(M._providers) do
      if p.models then
        for alias, _ in pairs(p.models) do
          ...
          if not seen[full] then names[#names + 1] = full; seen[full] = true end
        end
      end
    end
  ...
  vim.ui.input({ prompt = "Provider name (e.g., 'myapi'): " }, function(name)
    ...
    vim.ui.input({ prompt = "API format (openai/anthropic/gemini/custom): " }, function(format)
      ...
      vim.ui.input({ prompt = "Base URL (e.g., https://api.example.com): " }, function(base_url)
        ...
        if format == "anthropic" then default_ep = "/v1/messages"
        elseif format == "gemini" then default_ep = "/v1beta/models/{model}:generateContent" end
        ...
        vim.ui.input({ prompt = "API endpoint [" .. default_ep .. "]: ", default = default_ep }, function(ep)
          ...
          vim.ui.input({ prompt = "API key env var (e.g., MY_API_KEY): " }, function(env_var)
            if not env_var or env_var == "" then return end

            vim.ui.input({ prompt = "Default model ID: " }, function(default_model)
              if not default_model or default_model == "" then return end

              vim.ui.input({ prompt = "Model aliases (alias=id,alias=id or Enter to skip): " }, function(aliases_str)
                local models = {}
                if aliases_str and aliases_str ~= "" then
                  for pair in aliases_str:gmatch("[^,]+") do
                    local a, id = pair:match("^%s*(%w+)%s*=%s*(.+)%s*$")
                    if a and id then models[a] = id end
                  end
                end
                ...
                vim.notify(string.format(
                  ...
                  name, name, name), vim.log.levels.INFO)
              end)
            end)
          end)
        end)
...
function M.switch(model_str)
  ...
  if backend == "claude_code" then
    ...
    pcall(function() require("dwight.tracker").set_model(model_str) end)
  ...
  if backend == "api" then
    ...
    if not key then
      vim.notify(string.format("[dwight] No API key for %s. Set %s env var.",
        resolved.provider_name, resolved.provider.api_key_env or "?"), vim.log.levels.WARN)
    end
  end
  ...
  pcall(function() require("dwight.tracker").set_model(resolved.model_id) end)

  vim.notify(string.format("[dwight] Switched to %s / %s (%s backend)",
    resolved.provider_name, resolved.model_id, backend), vim.log.levels.INFO)
end
...
function M.show_current()
  ...
  if backend == "claude_code" then
    ...
    vim.notify(string.format("[dwight] Backend: claude_code | Model: %s | %s",
      model, key_status), vim.log.levels.INFO)
    return
  end

  if backend == "opencode" then
    key_status = "✅ managed by opencode"
    vim.notify(string.format("[dwight] Backend: opencode | Model: %s | %s",
      resolved.model_id, key_status), vim.log.levels.INFO)
    return
  end
  ...
  vim.notify(string.format("[dwight] Backend: api | Provider: %s | Model: %s | %s",
    resolved.provider_name, resolved.model_id, key_status), vim.log.levels.INFO)
end

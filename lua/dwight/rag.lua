...
local function rg_search(pattern, max_results, callback)
  ...
  handle = uv.spawn("rg", {
    args = {
      ...
      "-m", tostring(max_results),
      "--type-add", "code:*.{ts,tsx,js,jsx,py,go,rs,lua,rb,java,kt,c,cpp,h,cs,swift}",
      "-t", "code",
      "--glob", "!node_modules", "--glob", "!.git", "--glob", "!dist",
      "--glob", "!build", "--glob", "!vendor", "--glob", "!__pycache__",
      pattern,
    },
...
local function rg_search_sync(pattern, max_results, timeout_ms)
  ...
  rg_search(pattern, max_results, function(out) result = out; done = true end)
...
local function extract_potential_symbols(text)
  ...
  for word in text:gmatch("[A-Z][%w_]+") do
    if not seen[word] and #word >= 3 then seen[word] = true; symbols[#symbols + 1] = word end
  end
  for word in text:gmatch("[%w_]+%.[%w_]+") do
    if not seen[word] then seen[word] = true; symbols[#symbols + 1] = word end
  end
  return symbols
end
        ...
        if file and lnum then
          ...
          if abs_file ~= current_file then
            ...
            if not seen_files[key] then
              ...
              if context then
                parts[#parts + 1] = string.format("--- %s (lines %d-%d) ---\n%s", file, start_l, end_l, context)
              end
            end
          end
        end

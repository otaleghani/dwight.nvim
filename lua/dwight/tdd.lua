...
function M._run_tests()
  ...
  runner.run_with_callback(M._session.test_cmd, function(result)
    ...
    vim.notify(string.format(
      ...
      icon, M._session.iterations, M._session.test_cmd,
      pass_count or "?", fail_count or "?"
...
function M._parse_test_counts(result)
  ...
  if pass then return pass, fail end

  -- pytest: 3 passed, 1 failed
  pass, fail = output:match("(%d+) passed.-(%d+) failed")
  if pass then return pass, fail end
  pass = output:match("(%d+) passed")
  if pass then return pass, "0" end

  -- Go: ok/FAIL + pass count
  if output:match("^ok") then return "all", "0" end
  if output:match("^FAIL") then return "?", "?" end
...
function M.get_tdd_context(selection)
  ...
  if M._session and M._session.last_result then
    ...
    if r.stdout ~= "" then parts[#parts + 1] = r.stdout end
    if r.stderr ~= "" then parts[#parts + 1] = r.stderr end
  end
...
function M._save_run(result)
  ...
  if result.stdout and result.stdout ~= "" then f:write("STDOUT:\n" .. result.stdout .. "\n\n") end
  if result.stderr and result.stderr ~= "" then f:write("STDERR:\n" .. result.stderr .. "\n") end
  f:close()
  return path
end
...
function M.browse_runs()
  ...
  if vim.fn.isdirectory(dir) ~= 1 then
    vim.notify("[dwight] No test runs yet. Use :DwightTDD first.", vim.log.levels.INFO); return
  end
  ...
  if handle then
    while true do
      ...
      if ftype == "file" and name:match("%.txt$") then files[#files + 1] = name end
    end
  end
  table.sort(files, function(a, b) return a > b end)  -- newest first

  if #files == 0 then
    vim.notify("[dwight] No test runs found.", vim.log.levels.INFO); return
  end
  ...
  if has_tel then
    ...
    local items = {}
    for i, name in ipairs(files) do items[i] = i .. ". " .. name end
    vim.ui.input({ prompt = table.concat(items, " | ") .. " > " }, function(c)
      if not c then return end
      local idx = tonumber(c)
      if idx and files[idx] then vim.cmd("edit " .. vim.fn.fnameescape(dir .. "/" .. files[idx])) end
    end)
  end
end
...
function M.stop()
  if not M._session then
    vim.notify("[dwight] No TDD session active.", vim.log.levels.INFO); return
  end
...
function M.is_active() return M._session ~= nil end

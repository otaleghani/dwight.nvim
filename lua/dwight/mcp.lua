...
local _msg_id = 0
local function next_id() _msg_id = _msg_id + 1; return _msg_id end
...
function M.start_server(name, config)
  if M._servers[name] then return M._servers[name] end
...
function M._process_buffer(server)
  while true do
    ...
    if #server.buffer < body_end then break end
...
function M.stop_server(name)
  ...
  pcall(function()
    if server.stdin then server.stdin:close() end
    if server.stdout then server.stdout:close() end
  end)
  M._servers[name] = nil
end

function M.stop_all()
  for name in pairs(M._servers) do M.stop_server(name) end
end
...
function M.list_resources(name, callback)
  local server = M._servers[name]
  if not server then callback(nil, "Server not running"); return end

  if server.resources_cache then
    callback(server.resources_cache); return
  end
  ...
  send_request(server, "resources/read", { uri = uri }, function(result, err)
    ...
    if result and result.contents and #result.contents > 0 then
      ...
      for _, c in ipairs(result.contents) do
        if c.text then text_parts[#text_parts + 1] = c.text end
      end
...
function M.server_names()
  local names = {}
  for k in pairs(M._servers) do names[#names + 1] = k end
  table.sort(names)
  return names
end
...
function M.status()
  ...
  if #names == 0 then
    vim.notify("[dwight] No MCP servers configured.", vim.log.levels.INFO); return
  end

	...
	for path, old_content in pairs(_step_snapshots) do
		...
		if old_content ~= new_content then
			...
			if first_diff then
				...
				diff_parts[#diff_parts + 1] = string.format(
					"@@ -%d,%d +%d,%d @@",
					ctx_start,
					ctx_end_old - ctx_start + 1,
					ctx_start,
					ctx_end_new - ctx_start + 1
				)
...
function M.parse_plan(text)
	...
	for line in text:gmatch("[^\n]+") do
		...
		if smoke_str then
			...
			if not target.smoke_tests then
				target.smoke_tests = {}
			end
	...
	if
		#orphan_step.actions > 0
		or #orphan_step.prompts > 0
		or orphan_step.verify
		or #orphan_step.delegates > 0
		or (orphan_step.smoke_tests and #orphan_step.smoke_tests > 0)
	then
...
function M.validate_plan(steps)
	...
	for _, step in ipairs(steps) do
		...
		for _, action in ipairs(step.actions) do
			if action.path then
				if action.verb == "create" then
					creates[action.path] = true
				end
				if action.verb == "edit" then
					edits[action.path] = true
				end
			end
		end
		...
		if #step.prompts > 2 then
			warnings[#warnings + 1] = label
				.. ": has "
				.. #step.prompts
				.. " prompts — split into smaller steps for reliability."
		end

		-- Too many create/edit files in one step (multi-file corruption risk)
		local write_count = 0
		for _ in pairs(creates) do
			write_count = write_count + 1
		end
		for _ in pairs(edits) do
			write_count = write_count + 1
		end
		if write_count > 2 and #step.prompts > 0 then
			warnings[#warnings + 1] = label
				.. ": modifies "
				.. write_count
				.. " files with prompts — split into one-file-per-step for reliability."
		end

		-- Smoke tests: check for `go build -o ... ./...` anti-pattern
		for _, smoke in ipairs(step.smoke_tests or {}) do
			if
				smoke.command:match("go%s+build%s+%-o%s+%S+%s+%./%.\\.%.")
				or smoke.command:match("go%s+build%s+%-o%s+%S+%s+%./%.%.%.")
			then
				errors[#errors + 1] = label
					.. ": smoke uses 'go build -o <file> ./...' — this fails with multi-package projects. Use 'go build -o <file> .' instead."
			end
		end
		...
		if step.smoke_tests and #step.smoke_tests > 0 and not step.on_fail then
			errors[#errors + 1] = label
				.. ": has smoke test(s) but no @dwight:on-fail — cannot recover from smoke failures."
		end
		...
		if #step.prompts > 0 then
			...
			if not has_write_action and #step.delegates == 0 then
				warnings[#warnings + 1] = label
					.. ": has prompt(s) but no create/edit action — prompt may have no target file."
			end
		end
	end
	...
	for path, count in pairs(all_created) do
		if count > 1 then
			warnings[#warnings + 1] = string.format(
				"File '%s' is created in %d steps — later steps should use 'edit' instead of 'create'.",
				path,
				count
			)
		end
	...
	if #steps > 6 then
		warnings[#warnings + 1] = string.format(
			"Plan has %d steps (recommended max 6). Consider using @dwight:delegate for sub-tasks.",
			#steps
		)
	end
				...
				for _, entry in ipairs(log._entries) do
					if entry.id == job_id then
						if
							...
							or entry.status == "parse_fail"
							or entry.status == "pre_check_fail"
						then
							failed = true
						end
						break
...
function M._run_prompt(prompt_def, target_path, callback)
	...
	if target_path then
		...
		if not check_path:match("^/") then
			check_path = vim.fn.getcwd() .. "/" .. check_path
		end
		if vim.fn.filereadable(check_path) == 1 then
			...
			elseif line_count > 50 then
				...
				instructions = "IMPORTANT: You are editing an existing file with "
					.. line_count
	...
	local has_files_in_context = prompt_def.extra_context and prompt_def.extra_context:match("<related_files>")
	if mode.inject_run_output and target_path and not prompt_def._skip_companion and not has_files_in_context then
		...
		if prompt_def._is_agent and target_path and not prompt_def._skip_build_gate then
			run_build_gate(filepath, function(gate_result)
				if not gate_result.ok and not gate_result.skipped then
					...
					vim.notify(
						string.format(
							"[dwight] 🚧 Build gate failed for %s (exit %d) — verify loop will fix.",
							target_path,
							gate_result.exit_code or -1
						),
						vim.log.levels.WARN
					)
				...
				if not file_has_content(filepath) then
					if not prompt_def._is_retry then
						vim.notify(
							"[dwight] ⚠️ File empty after prompt — retrying with explicit instruction…",
							vim.log.levels.WARN
						)
						local retry_prompt = {
					...
					vim.notify(
						string.format("[dwight] ⚠️ Target file still empty after retry: %s", target_path),
						vim.log.levels.WARN
					)
					callback({
						filename = filepath,
...
function M._extract_error_files(output)
	...
	for line in output:gmatch("[^\n]+") do
		...
		if not file then
			file = line:match("^%s*([%w_/%-%.]+%.[%w]+):%d+:")
		end
		if not file then
			file = line:match("^%s*([%w_/%-%.]+%.[%w]+)%(%d+,%d+%):")
		end
		if not file then
...
function M._gather_files_context(paths, exclude_path)
	...
	for _, path in ipairs(paths) do
		...
		if exclude_path and path == exclude_path then
			goto continue
		end
		local full = cwd .. path
		...
		if f then
			...
			if content and #content < 10000 then -- skip huge files
				...
				parts[#parts + 1] =
					string.format('<file path="%s">(too large, %d bytes)</file>', path, #(content or ""))
			end
		end
		::continue::
	end
			...
			run_command_async(smoke.command, function(result)
				...
				if result.ok then
					...
					if not step.on_fail then
						qf_entries[#qf_entries + 1] =
							{ text = "⚠️ No @dwight:on-fail — cannot fix smoke failure", type = "W" }
					...
					local smoke_context = string.format(
						...
						smoke.command,
						result.exit_code,
						(failed_output or ""):sub(1, 2000)
					)

					on_fail_prompt.instructions = smoke_context
						.. (on_fail_prompt.instructions or "Fix the smoke test failure.")
					...
					if retry_level >= 2 then
						...
						for _, f in ipairs(error_files) do
							if not seen[f] then
								seen[f] = true
								context_files[#context_files + 1] = f
							end
						end
					...
					vim.notify(
						string.format(
							"[dwight] 🔥 Smoke retry %d/%d: fixing %s",
							retry_level,
							max_retries,
							on_fail_target
						),
					...
					M._run_prompt(on_fail_prompt, on_fail_target, function(entry, err)
						...
						if err then
							qf_entries[#qf_entries + 1] =
								{ text = "❌ smoke on-fail prompt failed: " .. err, type = "E" }
...
local function build_spec_check_prompt(step)
	...
	if #spec_parts < 2 then
		return nil
	end -- no prompts = nothing to check
		...
		run_command_async(step.verify.command, function(result)
			...
			if expect_fail then
				...
				if classification == "test_pass" then
					qf_entries[#qf_entries + 1] = {
						text = string.format(
							"⚠️ 🔍 %s: tests PASSED unexpectedly (exit %d)%s",
							label,
							result.exit_code,
							job_ref
						),
				...
				if classification == "test_failure" then
					qf_entries[#qf_entries + 1] = {
						text = string.format(
							"✅ 🔴 %s: tests fail as expected (exit %d)%s — red phase confirmed",
							label,
							result.exit_code,
							job_ref
						),
				...
				qf_entries[#qf_entries + 1] = {
					text = string.format("❌ 🔍 %s: BUILD ERROR (exit %d)%s", label, result.exit_code, job_ref),
					type = "W",
				}
			else
				-- ═══ GREEN PHASE ═══
			...
			if classification == "test_failure" and result.output then
				...
				for line in result.output:gmatch("[^\n]+") do
					...
					if line:match("got 404") or line:match("status 404") then
						four04_count = four04_count + 1
					end
				end
			...
			if all_404 then
				...
				for _, f in ipairs(step_files) do
					if
						f:match("server%.go$")
						or f:match("router%.go$")
						or f:match("routes%.go$")
						or f:match("app%.go$")
						or f:match("main%.go$")
						or f:match("server%.ts$")
						or f:match("app%.ts$")
						or f:match("routes%.ts$")
					then
			...
			if has_template_error then
				...
				for _, f in ipairs(step_files) do
					if f:match("server%.go$") or f:match("render%.go$") or f:match("handler") then
						local seen = false
						for _, ef in ipairs(error_files) do
							if ef == f then
								seen = true
								break
							end
			...
			elseif
				classification == "test_failure" and (not user_mode or user_mode == "code" or user_mode == "fix")
			then
			...
			if retry_level == 1 then
				...
				if all_404 or has_template_error then
					...
					vim.notify(
						string.format(
							"[dwight] 🔄 Retry 1/%d: /%s %s (%s → multi-file)",
							max_retries,
							on_fail_prompt.mode or "fix",
							on_fail_target,
							escalation_reason
						),
						vim.log.levels.INFO
			...
			elseif retry_level == 2 then
				...
				vim.notify(
					string.format(
						"[dwight] 🔄 Retry 2/%d: /%s %s with full context (%d files)",
						max_retries,
						on_fail_prompt.mode or "fix",
						on_fail_target,
						#step_files
				...
				vim.notify(
					string.format(
						"[dwight] 🔄 Retry 3/%d: /%s fresh-start multi-file mode",
						max_retries,
						on_fail_prompt.mode or "wire"
					),
					vim.log.levels.INFO
				)
			...
			M._run_prompt(on_fail_prompt, on_fail_target, function(entry, err)
				...
				if err then
					...
					if attempt <= max_retries then
						qf_entries[#qf_entries + 1] = {
							text = string.format(
								"🔄 LLM error on retry %d — will escalate to retry %d",
								attempt - 1,
								attempt
							),
							type = "W",
...
function M._step_file_sets(step)
	...
	for _, action in ipairs(step.actions) do
		...
		elseif action.verb == "move" then
			if action.src then
				reads[action.src] = true
			end
			if action.dest then
				writes[action.dest] = true
			end
		end
	end
...
function M._compute_parallel_groups(steps)
	...
	for i, step in ipairs(steps) do
		...
		local is_barrier = sets.has_run or sets.has_delegate or sets.has_verify or sets.has_smoke or #step.prompts > 0 -- prompts are slow and complex, serialize them
		...
		if is_barrier then
			...
			if conflicts then
				...
				for path in pairs(sets.write) do
					current_writes[path] = true
				end
				for path in pairs(sets.read) do
					current_reads[path] = true
				end
			end
		end
	end
...
function M._execute_steps(steps, depth, callback, on_step, on_entry, opts)
	...
	local function journal_entry(step, outcome, verify_output)
		...
		for _, action in ipairs(step.actions) do
			if action.verb == "create" then
				actions_summary[#actions_summary + 1] = "created " .. (action.path or "?")
			elseif action.verb == "edit" then
				actions_summary[#actions_summary + 1] = "edited " .. (action.path or "?")
			elseif action.verb == "read" then
				local paths = action.paths or {}
				actions_summary[#actions_summary + 1] = "read " .. table.concat(paths, ", ")
			elseif action.verb == "run" then
				actions_summary[#actions_summary + 1] = "ran " .. (action.command or "?")
			end
		end

		local entry = string.format("Step %d (%s): %s.", step.number, step.title, outcome)
	...
	local function inject_journal(step)
		...
		local journal_text = string.format("PREVIOUS STEPS (%d of %d, most recent):\n", #recent_journal, #journal)
	...
	local function run_next_step()
		if step_idx > #steps or had_error then
			...
			if had_error and opts.replan_fn and not opts._replanned then
				...
				for i = step_idx + 1, #steps do
					remaining_steps[#remaining_steps + 1] = steps[i]
				end
		...
		if on_step then
			pcall(on_step, step_idx, #steps, step.title)
		end
		...
		local function after_prompts()
				...
				M._run_verify_loop(step, qf_entries, function(updated_entries, verify_failed)
					...
					local function after_spec_check(drift_note)
						if drift_note then
							...
							journal[#journal + 1] =
								string.format("Step %d (%s): DRIFT WARNING — %s", step.number, step.title, drift_note)
						end
						...
						if #smoke_tests > 0 then
							...
							M._run_smoke_loop(step, qf_entries, function(smoke_entries, smoke_failed)
								...
								for i = pre_smoke_count + 1, #qf_entries do
									if qf_entries[i].text and not qf_entries[i].text:match("^%[Step ") then
										qf_entries[i].text =
											string.format("[Step %d] %s", current_step_num, qf_entries[i].text)
									end
									if on_entry then
										pcall(on_entry, qf_entries[i])
									end
								...
								if smoke_failed then
									had_error = true
									journal[#journal + 1] =
										journal_entry(step, "❌ FAILED (smoke)", "smoke test failed")
									callback(qf_entries, had_error, current_step_num, journal)
									return
								end
			...
			if #step.delegates > 0 then
				...
				local function next_delegate()
					...
					M._run_delegate(
						step.delegates[delegate_idx],
						depth,
						qf_entries,
						function(updated_entries, del_error)
							qf_entries = updated_entries
							if del_error then
								had_error = true
								callback(qf_entries, had_error, current_step_num, journal)
...
function M._run_post_review(steps, qf_entries, on_entry, callback)
	...
	require("dwight.skills")._run_llm(prompt, function(raw, code)
		...
		if not multi_files then
			...
			if #multi_files == 0 then
				multi_files = nil
			end
		end

		if multi_files and #multi_files > 0 then
			log.finish(job_id, "success", raw, string.format("Post-review: %d file fix(es)", #multi_files), nil)
			...
			if verify_cmd then
				...
				run_command_async(verify_cmd, function(result)
					...
					if result.ok then
						...
						local fail_e = {
							text = string.format(
								"❌ 🔎 Post-review: fixes broke build (exit %d)%s — manual fix needed",
								result.exit_code,
								job_ref
							),
							type = "W",
...
function M.execute(steps, opts)
	...
	for _, s in ipairs(exec_steps) do
		...
		if s.verify then
			total_verifies = total_verifies + 1
		end
		...
		for _, group in ipairs(parallel_groups) do
			if #group > 1 then
				...
				for _, idx in ipairs(group) do
					...
					if #s.prompts > 0 or s.verify or #s.delegates > 0 or (s.smoke_tests and #s.smoke_tests > 0) then
						all_action_only = false
						break
					end
				end
				...
				if all_action_only then
					...
					local combined = {
						...
						title = string.format("Batch: %s + %d more", exec_steps[group[1]].title, #group - 1),
						actions = {},
						prompts = {},
						verify = nil,
						on_fail = nil,
	...
	for _, step in ipairs(exec_steps) do
		...
		if #step.actions > 0 then
			parts[#parts + 1] = #step.actions .. " action(s)"
		end
		if #step.prompts > 0 then
			parts[#parts + 1] = #step.prompts .. " prompt(s)"
		end
		if step.verify then
			parts[#parts + 1] = step.verify.expect_fail and "verify-fail" or "verify"
		end
		if #step.delegates > 0 then
			parts[#parts + 1] = #step.delegates .. " delegate(s)"
		end
		local smokes = step.smoke_tests or {}
		if #smokes > 0 then
			parts[#parts + 1] = #smokes .. " smoke(s)"
		end
		if step._batched_from then
			parts[#parts + 1] = "⚡batched(" .. table.concat(step._batched_from, ",") .. ")"
		end
		step_list[#step_list + 1] = string.format("  %d. %s — %s", step.number, step.title, table.concat(parts, ", "))
	end

	local parallel_note = batched_count > 0 and string.format(", %d batched", batched_count) or ""
	vim.notify(
		string.format(
	...
	M._execute_steps(exec_steps, 0, function(qf_entries, had_error, failed_step_idx, journal)
		...
		local function finalize(final_entries, final_error, final_failed_idx)
			...
			if final_error then
				vim.notify(
					"[dwight] ❌ Execution failed. Use :DwightAgentResume to retry or :DwightMultiUndo to revert.",
...
function M.run(opts)
	...
	if #steps == 0 then
		vim.notify(
			"[dwight] No @dwight: directives found. Use /plan to generate an executable plan first.",
			vim.log.levels.WARN
		)
		return
	end

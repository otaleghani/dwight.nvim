-- dwight/skill_fetch.lua (formerly docs.lua)
-- Fetch documentation from URLs and generate skills from them.
-- :DwightDocsFromURL fetches a URL and converts it into a .dwight/skills/ file.
		...
		vim.ui.input({ prompt = "Documentation URL: " }, function(url)
			...
			vim.ui.input({ prompt = "Focus area? " }, function(focus)
				...
				fetch_url(url, function(doc_text, err)
					...
					vim.notify(
						string.format("[dwight] %d chars fetched. Generating…", #doc_text),
						vim.log.levels.INFO
					)

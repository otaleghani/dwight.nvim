...
T.test("surrogate high: ED A0 80 (U+D800)", function()
	...
	T.eq(false, result:find(string.char(0xED), 1, true) ~= nil, "surrogate byte should be replaced")
end)
...
T.test("above max: F4 90 80 80 (U+110000)", function()
	...
	T.assert(
		not (result:byte(1) == 0xF4 and #result >= 2 and result:byte(2) == 0x90),
...
T.test("realistic: Go source with embedded surrogate", function()
	local code = 'func main() {\n\tfmt.Println("hello ' .. string.char(0xED, 0xA0, 0x80) .. '")\n}'
	...
	for i = 1, #result do
		T.assert(
			result:byte(i) ~= 0xED or (i + 1 <= #result and result:byte(i + 1) < 0xA0),

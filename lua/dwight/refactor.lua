...
function M._find_importers(feature_files, tree_lines)
  ...
  for _, f in ipairs(feature_files) do
    ...
    if pkg then feature_packages[pkg] = true end
  ...
  local skip_ext = {
    ...
    [".woff"] = true, [".lock"] = true, [".sum"] = true, [".mod"] = true,
  ...
  for _, rel_path in ipairs(tree_lines or {}) do
    if rel_path:match("/$") then goto continue end  -- skip dirs
    if feature_set[rel_path] then goto continue end  -- skip feature's own files

    local ext = rel_path:match("%.[^.]+$") or ""
    if skip_ext[ext] then goto continue end

    -- Only scan source files
    local is_source = ext:match("%.go$") or ext:match("%.py$") or ext:match("%.js$")
      or ext:match("%.ts$") or ext:match("%.tsx$") or ext:match("%.jsx$")
      or ext:match("%.lua$") or ext:match("%.rs$") or ext:match("%.rb$")
      or ext:match("%.java$")
    if not is_source then goto continue end
  ...
  for _, file_info in ipairs(feature.files) do
    ...
    if f then
      ...
      if content and content ~= "" then
        ...
        local sigs = ""
        if ts_ok then sigs = ts.minimap(full) or "" end
        parts[#parts + 1] = string.format(
          '<file path="%s">\n<signatures>\n%s\n</signatures>\n<source>\n%s\n</source>\n</file>',
          filepath, sigs, content)
      end
    end
  end
  parts[#parts + 1] = "</feature_files>"
  ...
  if #importers > 0 then
    ...
    for _, imp in ipairs(importers) do
      ...
      parts[#parts + 1] = string.format(
        ...
        imp.path, table.concat(imp.imports, ", "), sigs, content)
    end
...
function M.refactor(feature_name, prompt)
  ...
  vim.notify(string.format("[dwight] 🔄 Preparing refactoring for $%s (%d files)…",
  ...
  pcall(function()
    ...
    if scan.tree then tree_lines = scan.tree end
  end)
  ...
  if #importers > 0 then
    local imp_names = {}
    for _, imp in ipairs(importers) do imp_names[#imp_names + 1] = imp.path end
    importer_note = string.format(
      "\n⚠️ Found %d file(s) that import this feature: %s\nThese must be updated if the public API changes.\n",
      #importers, table.concat(imp_names, ", "))
    vim.notify(string.format("[dwight] 📦 Found %d importer(s) of $%s", #importers, feature_name),
      vim.log.levels.INFO)
  end
  ...
  local desc = feature.description
    and string.format("Description: %s", feature.description) or ""

  -- Build the full decomposition prompt
  local full_prompt = string.format(REFACTOR_PROMPT,
    feature_name, desc, prompt, context,
    feature_name, feature_name, feature_name)

  -- Log to DwightLog
  local log = require("dwight.log")
  local job_id = log._next_id()
  log.start(job_id, "refactor:plan", api.nvim_get_current_buf(), 0, 0,
    string.format("DwightRefactor: $%s — %s\n%s\n\n%s",
      feature_name, prompt, importer_note, full_prompt:sub(1, 4000)))
  ...
  local auto_request = string.format(
    "Refactor feature $%s: %s%s",
    feature_name, prompt, importer_note)

  -- Inject the refactoring context into DwightAuto's decomposition
  -- by using the _decompose callback directly
  vim.notify(string.format("[dwight] 🔄 Decomposing refactoring plan for $%s…", feature_name),
    vim.log.levels.INFO)
  ...
  require("dwight.skills")._run_llm(full_prompt, function(raw, code)
    ...
    auto._show_decomposition(auto_request, tasks, function(approved_tasks)
      ...
      vim.notify(string.format("[dwight] 🔄 Starting refactoring: %d tasks", #approved_tasks),
        vim.log.levels.INFO)

...
function M.collect()
	...
	if log_ok and log._entries then
		for _, entry in ipairs(log._entries) do
			...
			if s == "success" then
				log_summary.success = log_summary.success + 1
			elseif s == "error" then
				log_summary.error = log_summary.error + 1
			elseif s == "parse_fail" then
				log_summary.parse_fail = log_summary.parse_fail + 1
			elseif s == "running" then
				log_summary.running = log_summary.running + 1
			end
			...
			if s == "success" then
				log_modes[mode].success = log_modes[mode].success + 1
			elseif s == "error" or s == "parse_fail" then
				log_modes[mode].failure = log_modes[mode].failure + 1
			end
			...
			if feat_from_mode and feat_from_mode ~= "" then
				...
				log_features_from_mode[feat_from_mode].invocations = log_features_from_mode[feat_from_mode].invocations
					+ 1
				if s == "success" then
					log_features_from_mode[feat_from_mode].success = log_features_from_mode[feat_from_mode].success + 1
				elseif s == "error" or s == "parse_fail" then
					log_features_from_mode[feat_from_mode].failure = log_features_from_mode[feat_from_mode].failure + 1
				end
			end
		end
	end
	...
	table.sort(models, function(a, b)
		return (a.stats.cost or 0) > (b.stats.cost or 0)
	end)
...
function M.show(target)
	...
	if #stats.daily > 0 then
		...
		for i = 1, show_days do
			if stats.daily[i].cost > max_cost then
				max_cost = stats.daily[i].cost
			end
			if stats.daily[i].invocations > max_inv then
				max_inv = stats.daily[i].invocations
			end
		end

		lines[#lines + 1] = string.format("  %-12s %8s %6s %6s %5s  %-20s", "Date", "Cost", "Calls", "OK", "Fail", "")
		lines[#lines + 1] = "  " .. string.rep("─", 65)

		for i = 1, show_days do
			local d = stats.daily[i]
			local sr = d.success + d.failure > 0 and string.format("%.0f%%", d.success / (d.success + d.failure) * 100)
				or "–"
			lines[#lines + 1] = string.format(
				"  %-12s %8s %6s %6d %5d  %s  %s",
				d.date,
				fmt_cost(d.cost),
				fmt_num(d.invocations),
				d.success,
				d.failure,
				bar(d.cost, max_cost, 15),
				sr
			)
		end
		lines[#lines + 1] = ""
	...
	if #stats.features > 0 then
		...
		for _, f in ipairs(stats.features) do
			local rate = f.success + f.failure > 0
					and string.format("%.0f%%", f.success / (f.success + f.failure) * 100)
				or "–"
	...
	if #stats.modes > 0 then
		...
		for _, m in ipairs(stats.modes) do
			mode_lines[#mode_lines + 1] =
				string.format("  %-20s %6s  %s", m.name, fmt_num(m.count), bar(m.count, max_mode, 25))
		end
		...
		if #mode_lines > 10 then
			...
			for _, ml in ipairs(mode_lines) do
				lines[#lines + 1] = ml
			end
		end
		lines[#lines + 1] = ""
	...
	if total_tokens > 0 then
		...
		lines[#lines + 1] = string.format(
			"  Cost efficiency: %s per estimated hour saved",
...
function M.show_features()
	...
	if #stats.features == 0 then
		vim.notify(
			"[dwight] No per-feature data yet. Use :DwightIssue or agent commands to generate data.",
			vim.log.levels.INFO
		)
		return
	end
	...
	table.sort(by_cost, function(a, b)
		return a.cost > b.cost
	end)

	local by_calls = vim.deepcopy(stats.features)
	table.sort(by_calls, function(a, b)
	...
	for i, f in ipairs(by_cost) do
		...
		local rate = f.success + f.failure > 0 and string.format("%.0f%%", f.success / (f.success + f.failure) * 100)
			or "–"
		lines[#lines + 1] = string.format(
			"  %2d. $%-18s %8s  %5s calls  %5s success  %s tokens",
			i,
			f.name:sub(1, 18),
			fmt_cost(f.cost),
			fmt_num(f.invocations),
		...
		if f.first_seen or f.last_used then
			lines[#lines + 1] =
				string.format("      First: %s  |  Last: %s", f.first_seen or "–", f.last_used or "–")
		end
	end
	lines[#lines + 1] = ""
	...
	lines[#lines + 1] = string.format(
		"  Total: %d features  |  %s calls  |  %s cost",
		#stats.features,
		fmt_num(total_calls),
		fmt_cost(total_cost)
	)
...
function M.export(format)
	...
	if format == "json" then
		...
		for _, f in ipairs(stats.features) do
			export_data.features[#export_data.features + 1] = {
				...
				success = f.success,
				failure = f.failure,
				success_rate = f.success + f.failure > 0 and (f.success / (f.success + f.failure) * 100) or nil,
	...
	elseif format == "csv" then
		...
		if f then
			...
			for _, d in ipairs(stats.daily) do
				local rate = d.success + d.failure > 0
						and string.format("%.1f", d.success / (d.success + d.failure) * 100)
					or ""
				f:write(
					string.format("%s,%.4f,%d,%d,%d,%s\n", d.date, d.cost, d.invocations, d.success, d.failure, rate)
				)
			end
			f:close()
		...
		if f then
			...
			for _, feat in ipairs(stats.features) do
				local rate = feat.success + feat.failure > 0
						and string.format("%.1f", feat.success / (feat.success + feat.failure) * 100)
					or ""
				f:write(
					string.format(
						"%s,%d,%.4f,%d,%d,%s,%d,%d,%s,%s\n",
						feat.name,
						feat.invocations,
						feat.cost,
						feat.success,
						feat.failure,
						rate,
						math.floor((feat.chars_sent or 0) / 4),
						math.floor((feat.chars_received or 0) / 4),
						feat.first_seen or "",
						feat.last_used or ""
					)
				)
		...
		if #files_written > 0 then
			vim.notify(
				string.format(
					"[dwight] ✅ Exported %d CSV files to %s/\n  %s",
					#files_written,
					export_dir:match("[^/]+$"),
					table.concat(

...
- Error handling: always handle errors explicitly. Use `if err != nil { return ... }` patterns.
  ...
  smoke = function(cwd)
    local main_check = vim.fn.system("grep -rq 'func main()' " .. cwd .. "/*.go " .. cwd .. "/cmd/ 2>/dev/null; echo $?")
    if vim.trim(main_check) ~= "0" then return nil end
    ...
    if cfg.languages then
      for id, user_def in pairs(cfg.languages) do
        if _registry[id] then
          ...
          if not _registry[id].name then _registry[id].name = id end
          if not _registry[id].comment then _registry[id].comment = "//" end
        end
      end
    end
  end)
...
function M.ids()
  ...
  for id in pairs(get_registry()) do
    if id ~= "unknown" then ids[#ids + 1] = id end
  end
  table.sort(ids)
  return ids
end
...
function M.detect()
  ...
  local ok, ctx = pcall(function() return require("dwight.context").scan() end)
  if ok and ctx and ctx.type then return ctx.type end
  ...
  for _, c in ipairs(candidates) do
    for _, marker in ipairs(c.detect) do
      if marker:find("%*") then
        ...
        if #matches > 0 then return c.id end
      else
...
function M.lint_cmd(lang)
  local def = M.get(lang or M.detect())
  if not def.lint then return nil, nil end
...
function M.smoke_cmd(lang)
  local def = M.get(lang or M.detect())
  if not def.smoke then return nil end
  local cwd = vim.fn.getcwd()
...
function M.syntax_check(code, filetype, filepath)
  ...
  local lang_map = {
    ...
    lua = "lua", php = "php", sh = "shell", bash = "shell",
    ruby = "ruby", rb = "ruby",
  }
  local lang_id = lang_map[filetype] or lang_map[ext]
  if not lang_id then return true, nil end

  local def = M.get(lang_id)
  if not def or not def.syntax_check then return true, nil end

...
function M.list_sessions()
  ...
  while true do
    ...
    if ftype == "file" and name:match("%.jsonl$") then
      ...
      if f then
        for _ in f:lines() do event_count = event_count + 1 end
        f:close()
      end
...
local function fmt_duration(start_ts, end_ts)
  if not start_ts or not end_ts then return "" end
...
local function render_event(event, idx, total, session_start)
  ...
  elseif etype == "cc_usage" then
    ...
    lines[#lines + 1] = string.format("  Input: %s tokens  |  Output: %s tokens",
  ...
  elseif etype == "cc_final_cost" then
    ...
    local tok_in = event.input_tokens or 0
    local tok_out = event.output_tokens or 0
    lines[#lines + 1] = string.format("  Tokens: %s in / %s out",
      tok_in >= 1000 and string.format("%.1fk", tok_in / 1000) or tostring(tok_in),
      tok_out >= 1000 and string.format("%.1fk", tok_out / 1000) or tostring(tok_out))
...
function M.replay(path)
  ...
  local function render()
    ...
    lines[#lines + 1] = string.format("  Step %d / %d  [%s]  %s",
      current, #events, pbar,
    ...
    for i = start_idx, end_idx do
      ...
      lines[#lines + 1] = string.format("  %s %d. %s %s%s  %s",
        marker, i, rendered.icon, rendered.title,
        rendered.offset, rendered.time)
    ...
    for i = 1, current do
      ...
      if t == "cc_tool" then tools_so_far = tools_so_far + 1
      elseif t == "cc-thought" or t == "cc_thought" then thoughts_so_far = thoughts_so_far + 1
      elseif t == "cc_tool_result" and events[i].is_error then errors_so_far = errors_so_far + 1
      end
    end
    lines[#lines + 1] = string.format("  Progress: %d tools  |  %d thoughts  |  %d errors",
      tools_so_far, thoughts_so_far, errors_so_far)
  ...
  local function next_step() go_to(current + 1) end
  local function prev_step() go_to(current - 1) end

  local function next_tool()
    for i = current + 1, #events do
      if events[i].type == "cc_tool" then go_to(i); return end
    end
  ...
  local function first_step() go_to(1) end
  local function last_step() go_to(#events) end
  ...
  pickers.new({}, {
    ...
    finder = finders.new_table({
      ...
      entry_maker = function(s)
        local display = string.format("%-20s %3d events  %s",
          s.timestamp or "?", s.event_count, s.task_name)
        return {
          value = s,
          display = display,
      ...
      define_preview = function(self, entry)
        ...
        for i, event in ipairs(events) do
          ...
          lines[#lines + 1] = string.format("%3d. %s %s%s",
            i, rendered.icon, rendered.title, rendered.offset)
        ...
        api.nvim_buf_set_lines(self.state.bufnr, 0, -1, false,
          require("dwight.util").flatten_lines(lines))
    ...
    attach_mappings = function(prompt_bufnr)
      actions.select_default:replace(function()
        ...
        if entry then M.replay(entry.value.path) end
      end)
      return true
    end,
  }):find()
...
function M._pick_fallback(sessions)
  local items = {}
  for _, s in ipairs(sessions) do
    items[#items + 1] = string.format("%s  %d events — %s",
      s.timestamp or "?", s.event_count, s.task_name)
  end
  ...
  require("dwight.select").pick(items, {
    ...
    if idx then M.replay(sessions[idx].path) end
  end)
end

...
function M._system_notify(title, body, urgency)
  ...
  elseif vim.fn.executable("powershell.exe") == 1 then
    -- Windows/WSL
    cmd = "powershell.exe"
    args = { "-Command", string.format(
      "[System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms'); " ..
      "$n = New-Object System.Windows.Forms.NotifyIcon; " ..
      "$n.Icon = [System.Drawing.SystemIcons]::Information; " ..
      "$n.Visible = $true; " ..
      "$n.ShowBalloonTip(5000, '%s', '%s', 'Info')",
      title:gsub("'", "''"), body:gsub("'", "''")) }
  end

  if not cmd then return end  -- no notification tool available
...
function M._notify_progress(task_num, total, task_title)
  M._system_notify(
    string.format("Dwight [%d/%d]", task_num, total),
    string.format("✅ Completed: %s", task_title))
end
...
function M._git_checkpoint(task_num, total, title, status)
  ...
  if add_code ~= 0 then
    status.append("  ⚠️ Git add failed — skipping checkpoint")
    return false
  end
  ...
  if commit_code ~= 0 then
    status.append("  ⚠️ Git commit failed — skipping checkpoint")
    return false
  end
...
function M._git_rollback(status)
  ...
  if code ~= 0 then
    status.append("  ⚠️ Git rollback failed")
    return false
  end
...
local function run_gate_cmd(cmd, timeout_ms, callback)
  ...
  timer:start(timeout_ms or 120000, 0, function()
    ...
    if not done then
      ...
      vim.schedule(function() callback(-1, "timeout after " .. (timeout_ms / 1000) .. "s") end)
    end
  end)

  stdout_pipe:read_start(function(err, data) if not err and data then stdout_chunks[#stdout_chunks + 1] = data end end)
  stderr_pipe:read_start(function(err, data) if not err and data then stderr_chunks[#stderr_chunks + 1] = data end end)
end
...
function M._gate_lint(status, callback)
  ...
  status.append(string.format("  🧹 Lint gate: %s", lint_cmd))
  ...
  run_gate_cmd(lint_cmd, 60000, function(code, output)
    if code == 0 then
      ...
      -- Count issues
      local issue_count = 0
      for _ in output:gmatch("\n") do issue_count = issue_count + 1 end
      status.append(string.format("  ❌ Lint gate FAILED (%s, ~%d issue(s))", lint_bin or "linter", issue_count))
      local n = 0
      for line in output:gmatch("[^\n]+") do
        status.append("    " .. line)
        n = n + 1
        if n >= 8 then status.append("    ..."); break end
      end
...
function M._gate_tests(status, callback, baseline_failures)
  ...
  status.append(string.format("  🧪 Test gate: %s", test_cmd))
    ...
    if code == 0 then
      ...
      if #new_failures == 0 and #all_failures > 0 then
        status.append(string.format(
          "  ⚠️ Test gate: %d pre-existing failure(s), no NEW failures — passing",
          #all_failures))
        callback(true, output)
      else
        status.append("  ❌ Test gate FAILED (exit " .. code .. ")")
        ...
        for line in output:gmatch("[^\n]+") do
          status.append("    " .. line)
          n = n + 1
          if n >= 10 then status.append("    ..."); break end
        end
        callback(false, output)
      end
    end
...
function M._gate_coverage(status, callback, baseline_coverage)
  ...
  status.append(string.format("  📊 Coverage gate: %s", cov_info.cmd))
  ...
  run_gate_cmd(cov_info.cmd, 180000, function(code, output)
    ...
    if not new_coverage then
      status.append("  ⚠️ Coverage gate: couldn't parse coverage — skipping")
      callback(true, output)
      return
    end
    ...
    if delta >= -1.0 then  -- allow 1% tolerance for flaky coverage measurement
      status.append(string.format("  ✅ Coverage gate: %.1f%% (baseline %.1f%%, delta %+.1f%%)",
        new_coverage, baseline_coverage, delta))
      callback(true, output)
    else
      status.append(string.format("  ❌ Coverage gate FAILED: %.1f%% → %.1f%% (dropped %.1f%%)",
        baseline_coverage, new_coverage, -delta))
...
function M._gate_smoke(status, callback)
  ...
  status.append(string.format("  🔥 Smoke gate: %s", smoke.build))

  -- Step 1: Build
  run_gate_cmd(smoke.build, 60000, function(build_code, build_output)
    if build_code ~= 0 then
      status.append("  ❌ Smoke gate: build FAILED")
      local n = 0
      ...
      -- Cleanup
      if smoke.cleanup then pcall(function() vim.fn.system(smoke.cleanup) end) end
    ...
    if smoke.run then
      status.append(string.format("  🔥 Smoke gate: %s", smoke.run))
      run_gate_cmd(smoke.run, 15000, function(run_code, run_output)
        ...
        local panicked = run_output:match("panic:") or run_output:match("SIGSEGV")
          or run_output:match("fatal error:") or run_output:match("Traceback %(most recent")
          or run_output:match("Error: Cannot find module")
        if panicked then
          status.append("  ❌ Smoke gate: app panicked/crashed on startup")
          local n = 0
  ...
  if #tasks == 0 then
    ...
    local function try_heading(line)
      for _, pat in ipairs(heading_patterns) do
        ...
        if num then return tonumber(num), t end
      end
      return nil, nil
    end
...
function M._decompose(request, callback)
  ...
  pcall(function()
    ...
    if manifest then
      if #manifest > 1500 then manifest = manifest:sub(1, 1500) .. "\n..." end
  ...
  pcall(function()
    ...
    if xml and xml ~= "" then context_parts[#context_parts + 1] = xml end
    local names = features.names()
    if #names > 0 then context_parts[#context_parts + 1] = "Features: " .. table.concat(names, ", ") end
  end)
  ...
  pcall(function()
    ...
    if scan.tree and #scan.tree > 0 then
      for _, line in ipairs(scan.tree) do
        ...
        if #tree_lines >= 60 then break end
      end
  ...
  pcall(function()
    ...
    if full_ctx then context_parts[#context_parts + 1] = full_ctx end
    ...
    if feature_ctx then context_parts[#context_parts + 1] = feature_ctx end
  end)
  ...
  local function explore_file(rel_path)
    if total_exploration_chars >= MAX_EXPLORATION_CHARS then return nil end
  ...
  for word in request_lower:gmatch("%w+") do
    if #word > 3 then request_keywords[word] = true end
  end
    ...
    if not line:match("/$") then  -- skip directories
        ...
        pcall(function()
          ...
          if peek_f then
            ...
            if peek then
              ...
              for kw in pairs(request_keywords) do
                if peek_lower:find(kw, 1, true) then keyword_hits = keyword_hits + 1 end
              end
      ...
      if best_priority > 0 and not files_seen[line] then
        ...
        scored_files[#scored_files + 1] = {
          path = line, reason = best_reason, priority = best_priority
        }
      end
    end
  end
  ...
  table.sort(scored_files, function(a, b) return a.priority > b.priority end)
  ...
  for _, entry in ipairs(scored_files) do
    if entry.reason == "test patterns" then
      ...
      if test_count > 2 then goto continue end
    end
  ...
  for _, entry in ipairs(files_to_read) do
    ...
    if content then
      exploration_parts[#exploration_parts + 1] = string.format(
        ...
        entry.path, entry.reason, content)
    end
  end
  ...
  log.start(job_id, "auto:decompose", api.nvim_get_current_buf(), 0, 0,
  ...
  require("dwight.skills")._run_llm(prompt, function(raw, code)
    ...
    if #tasks == 0 then
      ...
      local retry_prompt = string.format(
        ...
        raw:sub(1, 3000), request:sub(1, 200))
      ...
      require("dwight.skills")._run_llm(retry_prompt, function(retry_raw, retry_code)
        ...
        log.finish(job_id, "success", retry_raw,
...
function M._write_session_log(opts)
  pcall(function()
    ...
    lines[#lines + 1] = string.format("**Tasks:** %d/%d completed", opts.completed_count or 0, opts.total_tasks or 0)
    lines[#lines + 1] = ""

    -- Git diff stat: summary of ALL changes made in this session
    pcall(function()
      -- Get diff stat from the pre-auto checkpoint to HEAD
      local diff_handle = io.popen("git diff --stat HEAD~" .. tostring(opts.completed_count or 0) .. "..HEAD 2>/dev/null")
      if diff_handle then
    ...
    if opts.sessions then
      for _, s in ipairs(opts.sessions) do
        if s.files_created then
          for _, f in ipairs(s.files_created) do all_created[f] = true end
        end
        if s.files_edited then
          for _, f in ipairs(s.files_edited) do all_edited[f] = true end
        end
      end
    end
    ...
    for _ in pairs(all_created) do has_files = true; break end
    if not has_files then for _ in pairs(all_edited) do has_files = true; break end end
    ...
    if opts.failed_task then
      ...
      lines[#lines + 1] = ""
      lines[#lines + 1] = string.format("Failed on task %d: %s",
        opts.failed_task.num or 0, opts.failed_task.title or "?")
...
function M._build_prev_context(completed_sessions)
  if not completed_sessions or #completed_sessions == 0 then return "" end
  ...
  for _, session in ipairs(completed_sessions) do
    ...
    local status_icon = session.had_error and "❌" or "✅"
    ...
    if (not created or #created == 0) and (not edited or #edited == 0) and session.journal then
      ...
      for _, j in ipairs(session.journal) do
        for file in (j or ""):gmatch("created ([%w_/%.%-]+)") do
          if not created_set[file] then created_set[file] = true; created[#created + 1] = file end
        end
        for file in (j or ""):gmatch("edited ([%w_/%.%-]+)") do
          if not edited_set[file] then edited_set[file] = true; edited[#edited + 1] = file end
        end
      end
    end
    ...
    if created and #created > 0 then
      ...
      for _, f in ipairs(created) do file_touch_counts[f] = (file_touch_counts[f] or 0) + 1 end
    end
    if edited and #edited > 0 then
      parts[#parts + 1] = "  Edited: " .. table.concat(edited, ", ")
      for _, f in ipairs(edited) do file_touch_counts[f] = (file_touch_counts[f] or 0) + 1 end
    end
  ...
  if #fragile > 0 then
    table.sort(fragile)
    parts[#parts + 1] = "\n⚠️ FRAGILE FILES (edited by multiple tasks — read before editing, minimal changes):"
...
function M._execute_task_agentic(task, task_num, total_tasks, master_request, status, prev_sessions, callback)
  ...
  status.append(string.format("🤖 [%d/%d] Agentic: %s", task_num, total_tasks, task.title))
  ...
  pcall(function()
    local features = require("dwight.features")
    local xml = features.build_project_context()
    if xml and xml ~= "" then project_context = project_context .. xml .. "\n" end
  end)
  pcall(function()
    local bootstrap = require("dwight.bootstrap")
    local scan = bootstrap.scan()
  ...
  pcall(function()
    local integration = require("dwight.integration")
    local full_ctx = integration.build_full_context()
    if full_ctx then integration_text = "\n\n" .. full_ctx end

    -- Feature-scoped context: if the task references specific features,
    -- inject their full signatures and source snippets
    local feature_ctx = integration.build_feature_context(
      task.description .. " " .. master_request)
    if feature_ctx then integration_text = integration_text .. "\n\n" .. feature_ctx end
  end)
  ...
  local task_description = string.format(
    ...
    task.description, task_num, total_tasks, master_request:sub(1, 100))
    ...
    on_complete = function(success, data)
      ...
      pcall(function()
        agent._extract_lessons({
          ...
          had_error = not success,
          timestamp = os.time(),
        }, data.journal or {}, function(lessons)
          if lessons and #lessons > 0 then
      ...
      if success then
        status.append(string.format("✅ [%d/%d] %s — done in %ds%s%s",
          task_num, total_tasks, task.title, duration, cost_str, iter_str))
      else
        status.append(string.format("❌ [%d/%d] %s — FAILED in %ds%s%s",
          task_num, total_tasks, task.title, duration, cost_str, iter_str))
        -- Surface the actual error reason so it's not silently swallowed
        if data.error and data.error ~= "" then
          status.append(string.format("  ⚠️ Reason: %s", data.error:sub(1, 200)))
        end
...
function M._run_loop(tasks, start_from, master_request, master_started, status, prev_sessions)
  ...
  pcall(function()
    local agentic = require("dwight.agentic")
    local test_cmd = agentic.get_test_command()
    if not test_cmd then return end
    ...
    if code ~= 0 then
      ...
      local count = 0
      for _ in pairs(baseline_failures) do count = count + 1 end
      if count > 0 then
        status.append(string.format("  ⚠️ %d pre-existing test failure(s) (will be ignored in gates)", count))
        for name, _ in pairs(baseline_failures) do
          status.append("    - " .. name)
        end
      else
        -- Tests failed but we couldn't parse test names — might be compile error
        status.append("  ⚠️ Baseline tests failed (couldn't parse test names — may be compile error)")
  ...
  pcall(function()
    local agentic = require("dwight.agentic")
    local cov_info = agentic.get_coverage_command()
    if not cov_info then return end
    ...
    if pct then
      ...
      status.append("  ⚠️ Couldn't parse baseline coverage — delta gate will be skipped")
    end
  end)
  ...
  local function run_next(idx)
    if idx > total then
      ...
      status.append("")
      status.append("📊 Usage breakdown:")
      for _, s in ipairs(completed_sessions) do
        local task_cost = s.cost or 0
        local cost_str = task_cost > 0 and string.format("$%.2f", task_cost) or "—"
        local icon = s.had_error and "❌" or "✅"
        status.append(string.format("  %s Task %d: %s (%s)",
          icon, s.task_num or 0, s.title or "?", cost_str))
      end
      status.append("  ──────────────────────")
      ...
      if tokens.total > 0 then
        ...
        status.append(string.format("  Tokens: %s in / %s out (%s total)",
          fmt_tok(tokens.input), fmt_tok(tokens.output), fmt_tok(tokens.total)))
      end
      ...
      pcall(function()
        ...
        if gh._active_issue then
          vim.defer_fn(function() gh.maybe_offer_pr() end, 1000)
        end
      end)

      -- Write session summary log
      local cfg_ok, cfg = pcall(function() return require("dwight").config end)
      ...
      vim.notify(string.format(
        "[dwight] 🎉 DwightAuto: all %d tasks complete in %ds!",
        total, total_duration), vim.log.levels.INFO)
      return
    end
    ...
    M._execute_task(task, idx, total, master_request, status, completed_sessions, function(success, session_data)
      ...
      local function handle_failure(err_msg)
        ...
        pcall(function() agentic.abort() end)
        ...
        rollback_timer:start(3000, 0, function()
          ...
          vim.schedule(function()
            ...
            -- Write session summary log
            local cfg_ok2, cfg2 = pcall(function() return require("dwight").config end)
            M._write_session_log({
              success = false,
              request = master_request,
              duration = total_duration,
            ...
            vim.notify(string.format(
              ...
              idx, total, task.title), vim.log.levels.ERROR)
          end) -- vim.schedule
        end) -- rollback_timer
      end -- handle_failure
      ...
      if success then
        ...
        -- Show running total
        local running_tokens = status.session_tokens and status.session_tokens() or { input = 0, output = 0, total = 0 }
        local parts = {}
        if running_cost > 0 then parts[#parts + 1] = string.format("~$%.2f", running_cost) end
          ...
          if gate_passed then
            ...
            pcall(function()
              ...
              if #sync.added > 0 then
                status.append(string.format("  🏷️  Pragma sync: added @feature:%s to %d new file(s)",
                  feature_name or "?", #sync.added))
            ...
            if M._paused then
              status.append("")
              status.append(string.format("⏸️  PAUSED after task %d/%d. Use :DwightContinue to resume.", idx, total))
            ...
            status.append(string.format(
              "  ❌ Task %d reported success but verification pipeline failed!", idx))
...
function M._show_decomposition(request, tasks, callback)
  ...
  pcall(function() api.nvim_buf_set_name(buf, "dwight://auto-plan") end)
...
function M.auto(request)
  ...
  if existing and not existing.failed then
    vim.notify("[dwight] An auto session is already in progress. Use :DwightAutoResume or :DwightAutoCancel.",
      vim.log.levels.WARN)
    return
  end
  ...
  pcall(function()
    ...
    if #oversized > 0 then
      local names = {}
      for _, r in ipairs(oversized) do names[#names + 1] = "$" .. r.name end
      vim.notify(string.format(
        "[dwight] ⚠️ Large features detected: %s. Consider :DwightSplitFeature to improve reliability.",
        table.concat(names, ", ")), vim.log.levels.WARN)
    end
  end)
  ...
  M._decompose(request, function(tasks, err)
    ...
    vim.notify(string.format("[dwight] 🤖 Decomposed into %d sub-tasks. Review and <CR> to start.", #tasks),
      vim.log.levels.INFO)
...
function M._start_execution(request, tasks, start_from, prev_sessions)
  ...
  for i, task in ipairs(tasks) do
    local marker = i < start_from and "✅" or (i == start_from and "▶️" or "⬜")
    status.append(string.format("  %s %d. %s", marker, i, task.title))
  end

  M._system_notify("Dwight Auto Started",
    string.format("%d tasks: %s", #tasks, request:sub(1, 80)))
...
function M.resume(opts)
  ...
  if skip then
    ...
    vim.notify(string.format("[dwight] ⏭️  Skipping task %d/%d: %s",
      task_num, total, state.tasks[task_num].title), vim.log.levels.INFO)
    task_num = task_num + 1
  end
  ...
  msg_parts[#msg_parts + 1] = string.format("   ▶️  Next: %s", state.tasks[task_num].title)
...
function M.pause()
  ...
  M._paused = true
  vim.notify("[dwight] ⏸️  DwightAuto will pause after the current task completes.", vim.log.levels.INFO)
end
...
function M.continue(override_prompt)
  if M._pause_callback then
    vim.notify("[dwight] ▶️  Continuing DwightAuto...", vim.log.levels.INFO)
...
function M.skip()
  ...
  if M._pause_callback then
    vim.notify("[dwight] ⏭️  Skipping next task...", vim.log.levels.INFO)
...
- If the previous diff looks mostly correct, you can re-apply similar changes but fix the broken parts.
  ...
  vim.notify(string.format(
    ...
    task_num, total, task.title), vim.log.levels.INFO)
...
function M.cancel()
  ...
  M._paused = false
  M._pause_callback = nil
  pcall(function() require("dwight.agentic").abort() end)
...
function M.status()
  ...
  if state.failed then
    if state.retry_context then
      parts[#parts + 1] = "  Use :DwightAutoRetry to retry with error context, :DwightAutoResume to restart clean, :DwightAutoSkip to skip."
    else

...
local function server_supports(bufnr, method)
	...
	if not checker then
		return true
	end -- unknown method: try anyway

	for _, client in ipairs(clients) do
		if checker(client) then
			return true
		end
	end
	return false
...
local function lsp_request_sync(bufnr, method, params, timeout_ms)
	if not server_supports(bufnr, method) then
		return nil
	end
...
function M.format_context(ctx)
	...
	if #ctx.diagnostics > 0 then
		...
		for _, d in ipairs(ctx.diagnostics) do
			parts[#parts + 1] = string.format("  Line %d [%s] (%s): %s", d.line, d.severity, d.source, d.message)
		end
	end
	...
	if #ctx.symbols > 0 then
		...
		for _, s in ipairs(ctx.symbols) do
			...
			parts[#parts + 1] =
				string.format("  %s%s: %s%s", indent, s.kind, s.name, s.line and (" (line " .. s.line .. ")") or "")
		end
	end

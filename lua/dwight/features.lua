...
local function has_any_pragma(text)
	return text:match("@feature:")
		or text:match("@feature%-no%-docs")
		or text:match("@%s*project")
		or text:match("@%s*constraint")
		or text:match("@%s*stack")
		or text:match("@%s*convention")
		or text:match("@docs:")
		or text:match("@docs%-title:")
end
...
local function extract_dependencies(lines)
	...
	for _, line in ipairs(lines) do
		...
		if not mod then
			mod = trimmed:match("^from%s+([%w_%.]+)%s+import")
		end
		if not mod then
			mod = trimmed:match("^import%s+([%w_%.]+)")
		end
		-- Go: import "..."
		if not mod then
			mod = trimmed:match("^import%s+[\"']([^\"']+)[\"']")
		end
		-- Rust: use ...
		if not mod then
			mod = trimmed:match("^use%s+([%w_:]+)")
		end
		-- Lua: require("...")
		if not mod then
			mod = trimmed:match("require%s*%(?[\"']([^\"']+)[\"']")
		end
	...
	if vim.fn.executable("rg") == 1 then
		...
		for filepath in output:gmatch("[^\n]+") do
			...
			if filepath ~= "" then
				...
				for _, p in ipairs(feats) do
					all_features[#all_features + 1] = p
				end
				for _, p in ipairs(projs) do
					all_proj[#all_proj + 1] = p
				end
				for _, p in ipairs(docs) do
					all_docs[#all_docs + 1] = p
				end
				for k, v in pairs(no_docs) do
					all_no_docs[k] = v
				end
			end
		end
...
function M.pick()
	...
	if #feature_names == 0 then
		vim.notify(
			"[dwight] No features found. Add // @feature:name pragmas to your source files.",
			vim.log.levels.INFO
		)
		return
	end
	...
	if has_telescope then
		...
		vim.ui.input({ prompt = table.concat(items, " | ") .. " > " }, function(choice)
			...
			if idx and feature_names[idx] then
				M.show_virtual(feature_names[idx])
			end
		end)
	end
end
...
function M.build_project_context()
	...
	for _, p in ipairs(proj_pragmas) do
		if not grouped[p.pragma_type] then
			grouped[p.pragma_type] = {}
		end
	...
	if not has_pragmas and #feature_names == 0 then
		return nil
	end
	...
	if #feature_names > 0 then
		...
		for _, name in ipairs(feature_names) do
			...
			if feature then
				...
				local file_count = #feature.files
				parts[#parts + 1] =
					string.format('    <feature name="%s" files="%d">%s</feature>', name, file_count, desc:sub(1, 120))
			end
		end
		parts[#parts + 1] = "  </features>"
	end
...
function M.show_project_context()
	...
	if not xml and md_info == "" then
		vim.notify(
			"[dwight] No project context found. Add @project, @constraint, @stack, @convention pragmas to your code.",
			vim.log.levels.INFO
		)
		return
	end
...
function M.build_docs_structure()
	...
	for _, name in ipairs(doc_features) do
		...
		parts[#parts + 1] = string.format(
			'  <page path="features/%s.md" title="%s" feature="%s" />',
			name,
			desc:sub(1, 80):gsub('"', "'"),
			name
		)
	...
	for route, entries in pairs(route_groups) do
		...
		parts[#parts + 1] = string.format(
			'  <page path="%s.md" title="%s" source="%s" />',
			route,
			title:gsub('"', "'"),

...
function M.abort()
  if M._active_handle and not M._active_handle:is_closing() then
    pcall(function() M._active_handle:kill("sigterm") end)
...
local function detect_language()
  local ok, lang = pcall(function() return require("dwight.languages").detect() end)
...
local function log_event(event)
  if not session_log_file then return end
  pcall(function()
...
local function close_session_log()
  if session_log_file then
    pcall(function() session_log_file:close() end)
...
Do NOT just describe what you would do. Actually DO it right now.
  ...
  if lang_hint ~= "" then parts[#parts + 1] = "\n" .. lang_hint end
...
BACKENDS.claude_code = {
  ...
  new_parser = function(on_status, on_tool, log_ev)
    ...
    local function emit_tool(ti)
      ...
      elseif tt == "task_complete" or tt == "TodoComplete" then
        ...
        if ti.input then
          ...
          if p ~= "" then label = tt .. ": " .. p:sub(1, 60) end
        end
        on_tool("🔧 " .. tostring(label):sub(1, 80))
      end
    ...
    local function process_line(line)
      ...
      if not jok or type(event) ~= "table" then return end
        ...
        pcall(function()
          ...
          if etype == "assistant" then
              ...
              for _, block in ipairs(content) do
                if type(block) == "table" then
                  if block.type == "text" and block.text and block.text ~= "" then
                    on_status("💬 " .. block.text:sub(1, 300))
            ...
            elseif type(content) == "string" and content ~= "" then
              on_status("💬 " .. content:sub(1, 300))
            end
            local usage = (msg and msg.usage) or event.usage
            if usage then
              log_ev({ turn = -1, type = "cc_usage",
          ...
          elseif etype == "content_block_delta" then
            ...
            if delta and delta.type == "text_delta" and delta.text and delta.text ~= "" then
              on_status("💬 " .. delta.text:sub(1, 300))
            end
          ...
          elseif etype == "tool_result" then
            ...
            if type(content) == "table" then
              ...
              for _, b in ipairs(content) do
                if type(b) == "table" and b.text then ps[#ps + 1] = b.text
                elseif type(b) == "string" then ps[#ps + 1] = b end
              end
            ...
            if type(content) == "string" and content ~= "" then
              -- Detect test results
              local is_test = last_command and (
                last_command:match("test") or last_command:match("pytest")
                or last_command:match("jest") or last_command:match("vitest"))
              local is_build = last_command and (
                last_command:match("build") or last_command:match("compile")
                or last_command:match("tsc") or last_command:match("cargo check"))
              ...
              elseif is_build then
                local fail = content:match("FAIL") or content:match("error")
                  or content:match("Error") or (event.is_error)
                if fail then
              ...
              last_command = nil
              log_ev({ turn = -1, type = "cc_tool_result",
          ...
          elseif etype == "result" then
            ...
            if type(rt) == "string" and rt ~= "" then final_result = rt end
            if event.cost then
              log_ev({ turn = -1, type = "cc_final_cost",
          ...
          elseif etype == "error" then
            ...
            if type(emsg) == "table" then emsg = emsg.message or vim.inspect(emsg) end
            on_status("❌ CC: " .. tostring(emsg):sub(1, 200))
          end
        end)
      end)
    end
    ...
    return {
      ...
      get_result = function() return final_result end,
    }
  end,
  ...
  new_parser = function(on_status, on_tool, log_ev)
    ...
    return {
      ...
      get_result = function()
        ...
        if last_para and #last_para > 20 then return last_para end
...
local function spawn_backend(backend_name, opts)
  ...
  pcall(function()
    ...
    dlog.start(session_job_id, "agentic:" .. backend_name,
      api.nvim_get_current_buf(), 0, 0,
      string.format("═══ %s Session ═══\nBackend: %s\nTask: %s",
        backend_name, backend_name, (opts.task or "?"):sub(1, 300)))
  ...
  -- Track model
  pcall(function() require("dwight.tracker").set_model(backend_name) end)
  ...
  if env then spawn_opts.env = env end
  ...
  handle = uv.spawn(bin, spawn_opts, function(code)
    ...
    M._active_pid = nil
    M._active_backend = nil
    if timeout_timer and not timeout_timer:is_closing() then timeout_timer:close() end
    timeout_timer = nil
    ...
    vim.schedule(function()
      ...
      log_event({ turn = -1, type = "finish", exit_code = code,
        duration = duration, output_len = #output })
      close_session_log()
      ...
      if code ~= 0 then
        ...
        on_complete(false, {
          error = error_msg, summary = error_msg,
          journal = stderr_lines, duration = duration,
        })
        return
      end
      ...
      if summary == "" then
        ...
        local last_para = output:match("\n\n([^\n]+)%s*$")
        if last_para and #last_para > 20 then summary = last_para end
      end
  ...
  if not handle then
    local err = string.format("Failed to spawn '%s'. Is it installed? %s",
      bin, backend.install_hint or "")
    on_status("❌ " .. err)
    pcall(function()
      if session_job_id then
  ...
  timeout_timer:start(timeout_secs * 1000, 0, function()
    ...
    if handle and not handle:is_closing() then
      vim.schedule(function()
        on_status(string.format("⏰ %s timed out after %ds — killing",
          backend_name, timeout_secs))
      end)
      pcall(function() handle:kill("sigkill") end)
      local pid = M._active_pid
      if pid then pcall(function() uv.kill(-pid, "sigkill") end) end
    end
  end)
...
function M.available_backends()
  local names = {}
  for k in pairs(BACKENDS) do names[#names + 1] = k end
  table.sort(names)
  return names
end

...
function M.parse_xml(raw)
	if not raw or not raw:find("<changes>") then
		return nil
	end

	local changes_block = raw:match("<changes>(.-)</changes>")
	if not changes_block then
		return nil
	end
...
local function track_created_file(full_path)
	if not _undo_state.created_files then
		_undo_state.created_files = {}
	end
...
local function track_deleted_file(full_path)
	if not _undo_state.deleted_files then
		_undo_state.deleted_files = {}
	end
...
function M.undo_all()
	...
	if M.has_agent_snapshot() then
		...
		if ok then
			local parts = {}
			if restored > 0 then
				parts[#parts + 1] = string.format("restored %d file(s)", restored)
			end
			if deleted > 0 then
				parts[#parts + 1] = string.format("removed %d created file(s)", deleted)
			end
			if #parts == 0 then
				parts[#parts + 1] = "no changes found"
			end
	...
	local parts = {}
	if reverted > 0 then
		parts[#parts + 1] = string.format("reverted %d edit(s)", reverted)
	end
	if deleted > 0 then
		parts[#parts + 1] = string.format("removed %d created file(s)", deleted)
	end
	if restored > 0 then
		parts[#parts + 1] = string.format("restored %d deleted file(s)", restored)
	end
...
function M.apply_all(changes, opts)
	...
	if #qf_entries > 0 and opts.quickfix ~= false then
		...
		vim.cmd("copen")
		vim.notify(
			string.format("[dwight] ✅ Applied %d change(s). Quickfix open. :DwightMultiUndo to revert.", applied),
			vim.log.levels.INFO
		)
	end
...
function M.preview(changes, on_accept, on_reject)
	...
	for _, c in ipairs(changes) do
		counts[c.action] = (counts[c.action] or 0) + 1
	end
	if counts.create > 0 then
		summary_parts[#summary_parts + 1] = counts.create .. " new"
	end
	if counts.edit > 0 then
		summary_parts[#summary_parts + 1] = counts.edit .. " edit"
	end
	if counts.delete > 0 then
		summary_parts[#summary_parts + 1] = counts.delete .. " delete"
	end
	...
	pcall(vim.api.nvim_del_user_command, "DwightAccept")
	pcall(vim.api.nvim_del_user_command, "DwightReject")
	vim.api.nvim_create_user_command("DwightAccept", function()
		resolve(true)
	end, { desc = "Accept multi-file changes" })
	vim.api.nvim_create_user_command("DwightReject", function()
		resolve(false)
	end, { desc = "Reject multi-file changes" })
	...
	vim.schedule(function()
		...
		if qf_win and qf_win > 0 then
			...
			vim.keymap.set("n", "y", function()
				resolve(true)
			end, { buffer = qf_buf, desc = "Accept changes" })
			vim.keymap.set("n", "n", function()
				resolve(false)
			end, { buffer = qf_buf, desc = "Reject changes" })

...
function M.read()
  ...
  if not f then return { repos = {} } end
...
function M.add(repo_path, opts)
  ...
  if handle then
    remote = vim.trim(handle:read("*a") or ""); handle:close()
    if remote == "" then remote = nil end
  end

  -- Detect default branch
  local default_branch = nil
  handle = io.popen("git -C " .. vim.fn.shellescape(repo_path) .. " symbolic-ref refs/remotes/origin/HEAD 2>/dev/null")
  if handle then
    local ref = vim.trim(handle:read("*a") or ""); handle:close()
    default_branch = ref:match("refs/remotes/origin/(.+)")
  end
  if not default_branch then default_branch = "main" end
  ...
  if not ws.created then ws.created = os.date("%Y-%m-%dT%H:%M:%S") end
  M.write(ws)

  vim.notify(string.format("[dwight] ✅ Added repo: %s (%s)%s",
    name, repo_path, gh_repo and " → " .. gh_repo or ""),
    vim.log.levels.INFO)
  return true
end
...
function M.ensure_current()
  ...
  for _, r in ipairs(repos) do
    if r.path == cwd then return true end
  end
    ...
    if dwight_exists then
      ...
      local function scan_dir(dir, prefix)
        ...
        while true do
          ...
          if name:sub(1, 1) == "." or name == "node_modules" or name == "vendor"
            or name == ".git" or name == "dist" or name == "build" then
            goto continue
          end
          ...
          elseif ftype == "file" then
            ...
            if ext:match("^%.[a-z]+$") and not ext:match("%.json$")
              and not ext:match("%.ya?ml$") and not ext:match("%.toml$") then
                ...
                for _ = 1, 5 do
                  ...
                  if feat_name then
                    if not feature_map[feat_name] then
                      ...
                      desc = vim.trim(desc)
                      feature_map[feat_name] = { name = feat_name, files = {}, description = desc }
                    end
      ...
      table.sort(repo_features, function(a, b) return a.name < b.name end)
    end
...
function M.cross_repo_features()
  ...
  for _, repo_result in ipairs(scan) do
    for _, feat in ipairs(repo_result.features) do
      if not feature_repos[feat.name] then feature_repos[feat.name] = {} end
  ...
  table.sort(cross, function(a, b) return #a.repos > #b.repos end)
...
function M.fetch_all_issues(opts, callback)
  ...
  for _, repo in ipairs(gh_repos) do
    ...
    gh.list_issues(list_opts, function(issues, err)
      ...
      pending = pending - 1
      if pending == 0 then
        table.sort(results, function(a, b) return a.repo_name < b.repo_name end)
        callback(results)
      end
    end)
  end
  ...
  if #cross > 0 then
    ...
    for _, cf in ipairs(cross) do
      ...
      lines[#lines + 1] = string.format("  🔗 $%s — %s",
        cf.feature_name, table.concat(repo_names, ", "))
    end
  end
...
function M.add_interactive(path)
      ...
      if ftype == "directory" and name:sub(1, 1) ~= "." then
        ...
        if vim.fn.isdirectory(sib_path .. "/.git") == 1 then
          ...
          for _, r in ipairs(M.repos()) do
            if r.path == sib_path then already = true; break end
          end
          if not already then
  ...
  if #siblings == 0 then
    vim.ui.input({ prompt = "📦 Repo path to add: ", default = cwd }, function(input)
      if input and input ~= "" then M.add(input) end
    end)
    return
  end
...
function M.remove_interactive(name)
  ...
  require("dwight.select").pick(items, {
    ...
    if choice then
      ...
      if name_match then M.remove(name_match) end
    end
  end)
end
...
function M.show_features()
  ...
  for _, repo_result in ipairs(scan) do
    ...
    lines[#lines + 1] = string.format("  📦 %s%s — %d features",
      repo_result.repo_name, marker, #repo_result.features)

    if #repo_result.features > 0 then
      for _, feat in ipairs(repo_result.features) do
        local desc = feat.description ~= "" and (" — " .. feat.description) or ""
        lines[#lines + 1] = string.format("     $%-20s %d files%s",
          feat.name, feat.file_count, desc:sub(1, 40))
  ...
  if #cross > 0 then
    ...
    for _, cf in ipairs(cross) do
      ...
      for _, r in ipairs(cf.repos) do
        lines[#lines + 1] = string.format("     %s: %d files — %s",
          r.repo_name, r.file_count, r.description ~= "" and r.description or "(no description)")
      end
    end
    lines[#lines + 1] = ""
  end
...
function M.show_issues(opts)
  ...
  local repos = M.repos()
  local gh_repos = vim.tbl_filter(function(r) return r.gh_repo ~= nil end, repos)

  if #gh_repos == 0 then
    vim.notify("[dwight] No GitHub repos in workspace. Add repos with :DwightWorkspaceAdd.",
      vim.log.levels.WARN)
    return
  end

  vim.notify(string.format("[dwight] 📋 Fetching issues from %d repo(s)…", #gh_repos),
    vim.log.levels.INFO)
  ...
  M.fetch_all_issues(opts, function(results)
    ...
    for _, r in ipairs(results) do
      if r.error then errors[#errors + 1] = r.repo_name .. ": " .. r.error end
  ...
  pickers.new({}, {
    ...
    finder = finders.new_table({
      ...
      entry_maker = function(item)
        ...
        local display = string.format("%-12s #%-5d %s%s%s",
          item.repo_name:sub(1, 12), issue.number, issue.title, assignee, labels)
        return {
          value = item,
          display = display,
...
function M._show_issues_fallback(all_issues)
  ...
  for _, item in ipairs(all_issues) do
    items[#items + 1] = string.format("[%s] #%d: %s",
      item.repo_name, item.issue.number, item.issue.title)
  end
  ...
  vim.ui.select({ "Add all " .. #found .. " repos", "Pick individually", "Cancel" }, {
    ...
    if choice:match("Add all") then
      ...
      require("dwight.select").pick(items, {
        ...
        if idx then M.add(found[idx].path) end
      end)
    end
  end)
end
